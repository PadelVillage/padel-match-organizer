-- Padel Match Organizer - PROD assessment follow-up email scheduler.
-- Apply only to the PROD Supabase project after explicit PROMUOVI PROD authorization.
--
-- Scope:
-- - no first automatic email;
...
begin
  ...
  if coalesce(v_project_url, '') = '' or coalesce(v_publishable_key, '') = '' or coalesce(v_secret, '') = '' then
    ...
    set payload = v_payload || jsonb_build_object(
        'status', 'blocked',
        'error', 'PMO_ASSESSMENT_FOLLOWUP_VAULT_SECRET_MISSING',
        'updatedAt', now()
      ),
      synced_at = now()
  ...
  select net.http_post(
    ...
    body := jsonb_build_object(
      ...
      'limit', 20,
      'intervalHours', 48,
      'checkBeforeSend', v_action = 'routine-followup'
    ),
  ...
  values (
    'assessment_email',
    'assessment_followup_dispatch_last',
    v_payload,
    null,
    false,
    now()
