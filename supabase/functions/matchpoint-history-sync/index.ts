...
function bookingCloudKey(row: ParsedBooking, index: number, prefix = 'history') {
...
function parseBookingRows(rows: JsonMap[]) {
  ...
  return { bookings, occupancyRows, skipped };
}
...
function workbookRows(bytes: Uint8Array) {
  ...
  const missing = REQUIRED_HISTORY_COLUMNS.filter((name) => !normalizedHeaders.has(normalizeHeader(name)));
  if (missing.length) {
    return { ok: false as const, error: 'HISTORY_COLUMNS_MISSING', message: 'Il file non contiene le colonne minime dello storico prenotazioni.', missing, headers, sheetNames: workbook.SheetNames };
  }
...
function validateHistoryWorkbook(bytes: Uint8Array) {
  ...
  if (!result.bookings.length) {
    return {
      ok: false as const,
      error: 'NO_IMPORTABLE_HISTORY_ROWS',
      message: 'Il file storico e leggibile ma non contiene prenotazioni importabili.',
      sourceRows: parsed.rows.length,
      skipped: result.skipped,
      occupancyRows: result.occupancyRows,
    };
  }
  if (parsed.rows.length && result.bookings.length / parsed.rows.length < 0.2) {
    return {
      ok: false as const,
      error: 'LOW_HISTORY_ROW_RATIO',
      message: 'Troppe poche righe sembrano prenotazioni storiche valide: import bloccato.',
      sourceRows: parsed.rows.length,
  ...
  const dates = result.bookings.map((item) => item.data).filter(Boolean).sort();
  const today = todayIsoRome();
  const futureRows = result.bookings.filter((item) => item.data && item.data > today).length;
  return {
    ok: true as const,
...
function workerHistoryExportUrl(rawUrl: string) {
...
async function exportHistoryViaBrowserWorker(): Promise<MatchpointExport> {
  ...
  return {
    ...
    filename: clean(payload.filename) || `matchpoint-storico-${fromDate}-${toDate}.xlsx`,
...
async function loadExistingHistoryRecords(admin: any) {
  const records: any[] = [];
  for (let from = 0; ; from += PAGE_SIZE) {
    const { data, error } = await admin
      .from('pmo_cloud_records')
      .select('record_type,local_key,payload,deleted,synced_at')
      .eq('record_type', 'booking_history')
      .eq('deleted', false)
...
async function saveDiagnosticExport(_admin: any, exported: MatchpointExport, importedAt: string) {
  return {
    saved: false,
    reason: 'POLICY_NO_HISTORY_FILE_ARCHIVE',
...
async function saveFailureDiagnostic(admin: any, actor: StaffActor | null, importedAt: string, errorInfo: JsonMap) {
  ...
  const payload = {
    id: 'matchpoint_history_auto_diagnostic_last',
    type: 'storico prenotazioni',
    source: 'matchpoint_history_auto',
    importedAt,
  ...
  const { error } = await admin
    ...
    .upsert([{
      ...
      local_key: 'matchpoint_history_auto_diagnostic_last',
      payload,
      payload_hash: null,
      deleted: false,
...
async function saveValidationDiagnostic(
  ...
  const payload = {
    ...
    importedAt,
    actorEmail: actor?.email || '',
    code: validation.error || 'HISTORY_VALIDATION_FAILED',
    message: validation.message || 'Validazione file storico non superata.',
    validation: {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!validation.ok) {
      ...
      await logAudit(admin, actor, 'matchpoint_history_auto_import_blocked', {
        error: validation.error,
    ...
    const existingRecords = await loadExistingHistoryRecords(admin);
    ...
    records.push({
      record_type: 'matchpoint_data',
      local_key: 'matchpoint_history_auto_import_last',
    ...
    await logAudit(admin, actor, 'matchpoint_history_auto_import_success', {
      sourceRows: validation.sourceRows,
      importableRows: validation.importableRows,
      newRows,
      alreadyPresentRows,
    ...
    console.log(JSON.stringify({
      event: 'matchpoint_history_auto_import_error',
      importedAt,
      actorEmail: actor?.email || '',
      code: errorInfo.code,
      message: errorInfo.publicMessage,
    ...
    await logAudit(admin, actor, 'matchpoint_history_auto_import_error', {
      message: errorInfo.publicMessage,
      code: errorInfo.code,
      diagnosticSaved,
    }).catch(() => {});
    ...
    return errorResponse(500, 'MATCHPOINT_HISTORY_SYNC_FAILED', message, {
      diagnosticSaved,
      diagnostic: errorInfo.diagnostic || null,
    });
  }
