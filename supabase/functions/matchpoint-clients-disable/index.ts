...
// Errori LOGICI: ritentare NON aiuta. Tutto il resto è TRANSITORIO → ritentabile.
const NON_RETRYABLE_CODES = new Set([
  'CLIENT_NOT_FOUND', 'INVALID_CLIENT_CODICE', 'INVALID_CODICE',
  'CLIENT_DISABLE_VALIDATION', 'DISISCRIVI_BUTTON_NOT_FOUND',
  'MATCHPOINT_WORKER_SECRETS_MISSING', 'MATCHPOINT_CREDENTIALS_MISSING',
  'WORKER_NOT_CONFIGURED', 'WORKER_DISABLE_CLIENT_NOT_IMPLEMENTED',
]);
...
async function callWorkerDisableClient(opts: {
  workerUrl: string;
  workerApiKey: string;
  username: string;
  password: string;
  ...
  const endpoint = `${workerUrl}/disable-client`;
  ...
  if (res.status === 404 || res.status === 501) {
    throw workerFail('WORKER_DISABLE_CLIENT_NOT_IMPLEMENTED', 'Il worker non supporta /disable-client. Aggiornare il worker.');
  }
...
Deno.serve(async (req: Request) => {
  ...
  if (!hasPermission(actor, 'cloud_sync')) {
    return err(403, 'FORBIDDEN', 'Permesso cloud_sync richiesto per disiscrivere clienti su Matchpoint.');
  }
  ...
  try {
    workerResult = await callWorkerDisableClient({ workerUrl, workerApiKey, username, password, baseUrl, client });
  } catch (workerErr) {
  ...
  return ok({
    message: `Socio disiscritto su Matchpoint (codice ${codice})`,
    client,
    worker: workerResult,
  });
});
