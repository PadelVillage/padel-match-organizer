...
--
-- ⭐ Il guardiano sta sul DATABASE, non nell'app: guarda le righe nuove di
...
-- Una riga per token: la chiave primaria è la garanzia che la stessa scheda non
...
comment on table public.pmo_assessment_notifications is
  'Autovalutazioni già segnalate allo staff via email (assessment-notify-staff). Una riga per token.';
...
-- 🚨 IL PEZZO PERICOLOSO: senza questa riga, la prima passata manderebbe un'email per
...
insert into public.pmo_assessment_notifications (token, notified_at, nota)
select token, now(), 'preesistente all''accensione: email non inviata'
