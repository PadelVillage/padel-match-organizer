...
| Prevenzione prompt duplicati e handoff ripetuti | `docs/regola-id-prompt-anti-doppio.md` |
...
5. `docs/regola-id-prompt-anti-doppio.md`, se riceve prompt operativo o handoff;
6. documento della sezione su cui deve lavorare.
...
6. `docs/regola-id-prompt-anti-doppio.md`, se riceve prompt operativo o handoff;
7. documenti delle sezioni toccate dalla versione da promuovere.
...
## Regola anti-doppio per prompt e handoff

Ogni prompt operativo o handoff tra chat deve avere un `ID PROMPT` univoco e la `REGOLA ANTI-DOPPIO`.

Se una chat riceve due volte lo stesso `ID PROMPT`, non deve ripetere l'azione e deve rispondere solo:

```text
Prompt gia ricevuto: nessuna azione eseguita.
```

La regola completa vive in `docs/regola-id-prompt-anti-doppio.md` e deve essere richiamata nei prompt copiabili generati dalle chat.
...
## Regola sessioni browser di verifica Codex

Per ridurre i blocchi dei test browser sulla schermata login, le chat possono usare sessioni browser gia aperte con account staff dedicati alle verifiche Codex.

| Ambiente | Account | Permessi | Regola |
|---|---|---|---|
| TEST | `aprea.maurizio+codex.test@gmail.com` | Permessi TEST dedicati al collaudo | Usabile per verifiche post-login TEST; mai su PROD |
| PROD | `aprea.maurizio+codex.prod@gmail.com` | `Solo lettura` / `Solo consultazione` | Usabile solo per smoke test PROD non distruttivi |

Regole obbligatorie:

- Le password non devono mai essere scritte in chat, prompt, handoff, documenti o repository.
- `PMO-000948` resta il socio test operativo; non sostituisce l'account staff di login.
- Se la sessione browser e' disponibile, SVILUPPO TEST Admin e Promuovi PROD Admin possono usarla per verifiche post-login.
- Se la sessione e' scaduta o assente, la chat deve fermarsi e chiedere a Maurizio di fare login manuale.
- In PROD l'account Codex non deve eseguire azioni operative: niente modifica soci, gestione utenti, sync cloud, routine, invii email, Matchpoint, export/import dati, WhatsApp operativo o salvataggi.
- La verifica PROD con account Codex deve limitarsi a login, apertura menu/sezioni, controllo versione, console browser e assenza di errori bloccanti evidenti.
- Se una verifica richiede permessi superiori in PROD, la chat deve fermarsi e chiedere conferma esplicita; non deve elevare autonomamente l'account.
