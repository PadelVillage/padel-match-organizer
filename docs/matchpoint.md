...
Ultimo aggiornamento: 2026-06-01 23:30

## Nota TEST 2026-06-01: ID Matchpoint nei soci + pulizia doppioni legacy (supera v5.488)

Solo ambiente TEST (progetto Supabase `cudiqnrrlbyqryrtaprd`), funzione `matchpoint-clients-sync` versione 45. Nessuna modifica a PROD. Tre interventi, tutti collaudati in TEST:

1. **Aggancio ID interno Matchpoint (`memberId`)**: oltre al report clienti con il livello, la sync scarica anche il report `Listadoclientes` (colonna `Codice`) e riempie il campo `memberId` di ogni socio con il codice Matchpoint a 6 cifre (es. `000004`), abbinando per telefono/email. `Codice` NON viene usato come chiave di match (resta telefono/email/nome normalizzato, coerente con la funzione v18): viene solo letto e copiato in `memberId`. Collaudo TEST: 982 soci agganciati su 983; 1 non abbinato (telefono/email non combacianti) segnalato per revisione manuale. Il riepilogo salva `memberIdEnrichment` (matched/unmatched/unmatchedSample).

2. **Login HTTP non operativo -> entrambi i report via browser worker**: il login HTTP della Edge Function verso Matchpoint non funziona (la risposta torna su `Login.aspx?AspxAutoDetectCookieSupport=1`, con campo password ancora presente). Il flusso ha sempre funzionato perche' ripiega sul browser worker (Playwright), che fa login con un browser vero. Di conseguenza sia il report con il livello sia quello con il `Codice` vengono scaricati dal worker: il report Codice usa la modalita' worker `direct_clients` su `/clientes/Listadoclientes.aspx`. Secret opzionali per override: `MATCHPOINT_CODICE_PATH`, `MATCHPOINT_CODICE_EXPORT_TARGET`, `MATCHPOINT_CODICE_NAV_MODE` (default `direct_clients`). Nota: questo comporta due chiamate al worker per ogni sync (un report alla volta), quindi la sync clienti e' piu' lenta del solito.

3. **Pulizia automatica doppioni legacy**: dopo un import riuscito, i record `member` non-Matchpoint (es. `source=legacy_duplicate`, `memberId` tipo `PMO-xxxxxx`) che hanno gia' un gemello Matchpoint per lo stesso socio vengono marcati `deleted=true` (motivo `legacy_duplicate_superseded`). Due guardie di sicurezza: (A) si elimina solo se il record sopravvissuto e' davvero Matchpoint; (B) si elimina solo se il doppione e' privo di dati curati (preferenze, disponibilita', note, gruppi, ecc.). I doppioni legacy CON dati curati NON vengono cancellati: vengono conteggiati e segnalati in `legacyDuplicateReviewSample` per revisione manuale. Soft-delete reversibile, nessun hard-delete. Il riepilogo/audit salva `legacyDuplicateDeleted` e `legacyDuplicateReview`. Collaudo TEST: 1 doppione eliminato (`Maurizio Aprea` / `PMO-000948`), 0 in revisione; dopo la sync nessun socio attivo resta con `memberId` tipo `PMO-` (0 PMO attivi, 982 con codice Matchpoint, 1 senza id).

Questa nota **supera la decisione v5.488** ("`Maurizio Aprea` / `PMO-000948` e' il record da tenere"). Motivo: a maggio il record Matchpoint con l'ID interno corretto non esisteva ancora sul socio, quindi si teneva il record PMO; ora il record Matchpoint porta `memberId=000004`, percio' il vecchio `PMO-000948` e' un doppione ridondante ed e' corretto eliminarlo. Decisione confermata da Maurizio il 2026-06-01.

**ATTENZIONE / da allineare (intervento futuro)**: lo strumento manuale `Diagnostica conteggi soci` (note v5.485-v5.489) ragiona ancora con la vecchia decisione e, se lanciato, potrebbe proporre di RIPRISTINARE `PMO-000948`. Non e' automatico, quindi non combatte con la sync; ma se usato annullerebbe questa pulizia. Allinearlo (perche' non riproponga il ripristino dei doppioni gia' superati da un gemello Matchpoint) e' un intervento sull'app ancora da fare.
...
La diagnostica conteggi soci gestisce il duplicato `PMO-000948` / `aprea.maurizio@gmail.com` secondo la decisione funzionale approvata: `Maurizio Aprea` e' il record da tenere, mentre `Test Maurizio Autovalutazione` e' il duplicato da mandare in soft-delete. Se `Maurizio Aprea` era gia marcato deleted, il dry-run lo classifica come `Ripristino approvato` e il comando TEST applica il ripristino controllato senza hard-delete. Il dry-run SQL resta read-only.

Promossa in PROD il 2026-05-19 con app v5.488. Il file `supabase_pmo_member_count_audit_dry_run.sql` e' stato pubblicato nel repository come riferimento preparatorio ma non applicato a Supabase PROD; nessuna RPC dry-run e nessuna correzione dati e' stata eseguita durante la promozione. Il comando di applicazione resta bloccato lato app fuori da TEST.

## Nota TEST v5.487

La diagnostica conteggi soci mantiene sempre visibile il comando `Applica soft-delete approvati in TEST` dopo il dry-run. Se non ci sono record approvati, il bottone resta disabilitato e mostra il motivo. La tabella dei dettagli e' stata compattata accorpando contatti, chiavi e motivazione per evitare scroll orizzontale inutile.

## Nota TEST v5.486

La diagnostica conteggi soci recepisce due decisioni operative approvate:

- `Test di autovalutazione` duplicato di `Maurizio Aprea`: il record viene marcato `Soft-delete approvato` solo quando il collegamento e' tecnico, tramite PMO, email o telefono. Il socio test ufficiale `PMO-000948` / `aprea.maurizio@gmail.com` e' sempre preservato.
- `Tennis Up/Tennis App`: il record tecnico non Matchpoint puo essere marcato `Soft-delete approvato`; la sorgente Matchpoint resta mantenuta.

Il bottone `Applica soft-delete approvati in TEST` opera solo in TEST, richiede conferma manuale, non fa hard-delete e aggiorna i record cloud con `deleted=true`, `payload.active=false`, `payload.deletedAt`, `payload.cleanupReason` e, dove disponibile, `payload.mergedInto`.
...
E' stato preparato anche il file SQL `supabase_pmo_member_count_audit_dry_run.sql`: solo dry-run read-only, grant `authenticated`, nessun grant `anon`, nessun hard-delete e nessuna RPC di cleanup applicabile. Prima di qualunque procedura di pulizia serve approvazione esplicita di Maurizio sulla lista record.
...
- da versione funzione 45 (2026-06-01, solo TEST), la sync aggancia l'ID interno Matchpoint nei soci: scarica anche il report `Listadoclientes` (colonna `Codice`) e riempie `memberId` con il codice a 6 cifre, abbinando per telefono/email (`Codice` non e' chiave di match, coerente con la v18). Poiche' il login HTTP della Edge Function non e' operativo, sia il report con il livello sia quello con il `Codice` vengono scaricati dal browser worker; il report Codice usa la modalita' worker `direct_clients`. Secret opzionali: `MATCHPOINT_CODICE_PATH`, `MATCHPOINT_CODICE_EXPORT_TARGET`, `MATCHPOINT_CODICE_NAV_MODE`. Il riepilogo salva `memberIdEnrichment` (matched/unmatched/unmatchedSample). Comporta due chiamate al worker per sync (un report alla volta).
- da versione funzione 45 (2026-06-01, solo TEST), dopo un import riuscito la sync marca `deleted=true` (motivo `legacy_duplicate_superseded`) i doppioni `member` non-Matchpoint che hanno gia' un gemello Matchpoint per lo stesso socio, ma SOLO se (A) il record sopravvissuto e' Matchpoint e (B) il doppione e' privo di dati curati; i doppioni con dati curati vengono solo segnalati (`legacyDuplicateReviewSample`). Soft-delete reversibile, nessun hard-delete. Il riepilogo salva `legacyDuplicateDeleted` e `legacyDuplicateReview`. Questo supera la decisione v5.488 di tenere `PMO-000948` (vedi nota TEST 2026-06-01 in cima al documento).
