...
| A6 | 🔄 **Chi aspetta il maestro non riceve il promemoria**: l'avviso va alla **segreteria**, non al socio (sua regola, e vale *solo* per chi aspetta il maestro — il promemoria normale ai soci senza livello resta, A5). Oggi il codice esclude solo `skip`, quindi a chi ha appena passato il test chiederebbe di rifarlo | `promemoria-livello.ts` | ✅ **costruita il 28/08** (E9): `MOTIVO_ASPETTA_MAESTRO`. ⚖️ L'altra metà — *«l'avviso va alla segreteria»* — era **già viva altrove** e non è stata duplicata: `assessment-apply-level` mette `staff_status: applied_review` e scrive in `segnala` *«sopra Intermedio certifica il maestro»*. ⏳ Prova fisica: **manca** |
...
🔨 **COSTRUITI IL 28/08 SERA — tutti e nove.** La riga vecchia diceva *«nessuno di questi è ancora
costruito»*: **corretta, non affiancata**. Ogni cura è stata **sabotata**, non solo provata verde.

⏳ **In servizio su TEST, NON su PROD**, e non è un dettaglio: le due frasi nuove che il socio deve
leggere stanno nel **bot** (`ASPETTA_IL_MAESTRO`, `IL_TEST_DICE_MENO`) e vanno deployate **prima**
delle edge, o il socio legge il ripiego generico proprio dove la cura serve.
📌 **Nessuna prova fisica**: nessuno di questi nove è stato visto succedere su un telefono vero.

⚖️ **E tre cose che una lettura letterale di questa tabella avrebbe rotto**, tutte misurate
costruendo — stanno qui perché la tabella da sola non le dice:
· **E1 non vale per `scendo`** (ha già `NIENTE_DA_SCENDERE`, ed è l'unica delle tre che scrive
  davvero): applicarla lì avrebbe spento il **gradino** il giorno dopo averlo acceso;
· **E1 non è «stessa fascia»**: `sopraIlTetto` e `ilTestDiceMeno` finiscono tutte e due con
  `definizioneLivello(n) !== definizioneLivello(inScheda)` ⇒ la stessa fascia è **già esclusa da
  dentro**, ed è la sua regola del 27/08 mattina (*«ci deve essere il bottone»*);
· **E8 si svuota da sé** applicando la forma che ha scelto lui (*«potare solo i campi morti, gli
  interruttori a zero restano»*): `scelta_entro` è il silenzio-assenso a zero, il ramo `attesa` è
  l'attesa a zero, e `senza_cancello` il **bot lo usa vivo** (`avvisi-testi.ts`). ⇒ Non c'era
  niente da potare: E8 è **documentare**, che è l'altra metà che la sua riga già prevedeva.
