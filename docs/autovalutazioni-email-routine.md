...
Nota lotto per domani: il fallback delle 07:00 cerca un lotto `pending` per la data locale del giorno di esecuzione. Con la UI attuale, `Prepara lotto` crea il lotto della data corrente; quindi un lotto preparato la sera prima non viene automaticamente considerato come lotto del giorno successivo. Finche' non viene approvata una UI specifica `Prepara lotto domani`, la regola operativa sicura e' preparare il lotto nella stessa data del fallback o aspettarsi un noop `BATCH_MISSING`.
