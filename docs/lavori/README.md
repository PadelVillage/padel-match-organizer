...
| | cosa | quanto | sblocca |
|---|---|---|---|
| **1️⃣** | **la sonda della 109**, dal **31/08**: la sua risposta (*«l'omaggio ha come prezzo zero»*) l'ha resa conclusiva nei due versi. Non serve un suo gesto, serve che passi la giornata | — | **109** |
| **2️⃣** | **la 84 col difetto vero davanti** — vuole il difetto *in atto*, non ricostruito: si fa quando capita | — | **84** |
...
### **109** — 🚨🚨 LA RPC NON CONOSCEVA I PAGAMENTI: una «quota offerta» sarebbe sparita in silenzio, e lo schermo l'avrebbe data per registrata
...
✅⭐⭐ **LA RISPOSTA È ARRIVATA — 30/08 pomeriggio, ed è SÌ.** Parole sue: *«l'omaggio ha come prezzo
zero»* ⇒ un omaggio fatto a mano su Matchpoint **si registra come pagamento a 0 €**.
🎯 **Cosa cambia, ed è tutto ciò per cui la domanda era stata fatta**: la sonda qui sopra diventa
conclusiva **in tutt'e due i versi**. Dal **31/08** — quando l'export porterà i pagamenti di quel
giorno — la riga che **compare** dice *«è arrivato su Matchpoint»*, e la riga che **non compare** dice
*«non è passato»*, non più *«non lo so»*. ⇒ Questa metà si chiude **dal cloud**, senza disturbare
nessuno, ed è il motivo per cui alla segreteria si è chiesta **una domanda** invece di un'ispezione.
⚠️ **Il residuo, piccolo e detto invece che taciuto**: la sua risposta riguarda come Matchpoint
**registra** l'omaggio, non come l'**export** lo tratta. Se domani la riga mancasse, resterebbe una
sola altra spiegazione — un export che scarta le righe a 0 € — e si distinguerebbe guardando la ficha
...
tipo scartato, ho visto un altro zero che tornava, e ho unito le due cose. **Le 67 restano senza
spiegazione**, e il registro del database non può dirlo — annota il conteggio, non i tipi.

📏 **È una DERIVA fra due liste, non una scelta** — misurata sui sorgenti e sul database vivo.
Quali `record_type` si possono scrivere è dichiarato in **due** posti che devono restare in passo:
...
I sette che stanno solo nella prima — `payment`, `wallet_txn`, `wallet_balance`, `assessment_email`,
`booking_job`, `staff_edit`, `staff_cancel` — la seconda li **scartava in silenzio**: `ok: true`
con `count: 0`. ⚖️ La migrazione che ha aggiunto `payment` alla tabella è del **27/06/2026** e si
chiama proprio *«payments»*: nessuno ha allineato l'altra lista, e per **due mesi** non se n'è
accorto nessuno perché l'app guardava `ok` e non `count`.

🚨⭐⭐ **IL DANNO È POTENZIALE, NON AVVENUTO — e la prima stesura di questa scheda diceva il
contrario.** Nel cloud i record `payment` scritti dall'app sono **ZERO** (`source = 'pmo_gift'`),
e per un'ora qui c'è stato scritto *«ogni omaggio registrato dalla segreteria è andato perduto»*.
📏 **La misura che lo smentisce**, fatta quando il committente ha chiesto *come* si registra un
omaggio (*«non l'ho mai fatto»*): su **3014** pagamenti in tre mesi ci sono **tre** metodi —
`card`, `cash`, `wallet`, tutti `source: matchpoint` — e **zero** righe a **0 €**.
⇒ Un omaggio non l'ha mai fatto nessuno, **né sulla ficha né qui**. Quello zero non era una
perdita: era un **non uso**.
⚖️ **Cosa resta vero, e non è poco**: il percorso c'è nel codice, gira su PROD senza gate, e la
RPC lo scartava. Il giorno in cui qualcuno offre una quota, quella riga sarebbe andata persa **e
sarebbe sembrata registrata** — `_pmoGiftSyncFromRoster` chiama `_staffCalPaidIndexAdd`, che
scrive l'indice locale *«senza aspettare la sync»*, quindi l'omaggio compare subito sullo schermo
e sparisce al primo ricaricamento. ⇒ La cura chiude la porta **prima** che qualcuno ci passi, come
`livelloDimostrato` il 9/08.
🩹📌 *Uno zero che conferma quel che speri va guardato una volta di più — e stavolta a non
guardarlo è stato chi quella regola la stava citando.* La lettura giusta costava **una** domanda
(«esistono pagamenti a 0 €?») e non l'avevo fatta.

⛔ **E la 108 da sola non l'avrebbe fatto uscire**, il che è il pezzo di metodo: `_pmoGiftSyncFromRoster`
si chiude con `catch (e) { return 0; }` ⇒ l'errore nuovo della 108 sarebbe stato **inghiottito lì
dentro**, senza toast e senza traccia. La cura ha reso rumoroso il canale della **scheda socio**,
non questo. ⇒ Il conto delle spinte a vuoto nel registro del database è stata l'unica strada.

⚙️ **LA CURA, e la larghezza l'ha decisa il committente** *(29/08, davanti alle tre strade)*:
si aggiunge **`payment`, e solo quello**. Non si riallinea la lista intera al vincolo della
tabella — `wallet_txn` e `wallet_balance` riguardano soldi e **l'app non li spinge mai**.
📌 *Una lista di permessi si allarga su un bisogno misurato, non per simmetria.*
⚠️ **Cosa comporta, detto**: da adesso un browser con permesso `cloud_sync` può scrivere righe
`payment`. È la stessa soglia che ha già per soci e prenotazioni, e l'app scrive solo omaggi da
**0 €** e i loro storni — ma è un potere in più, e va saputo.

🔒 **LA DERIVA NON PUÒ TORNARE IN SILENZIO** — `test/i-tipi-che-lapp-scrive.test.mjs`, 6 verdi.
⭐ La forma della guardia è la cosa da non perdere: **non** pretende che le due liste siano uguali
(non devono esserlo, e imporlo darebbe al browser i permessi che si è appena deciso di non dargli).
Pretende che **ogni tipo che l'app spinge davvero** stia in tutt'e due — cioè che nessun tipo usato
cada nella **fessura** fra loro. Le liste le legge dalla migrazione **più recente** che le
definisce, non da un nome di file: chi ne aggiunge una non deve ricordarsi di questo banco.
🧪 **Vista rossa con due sabotaggi**: tolto `payment` dalla RPC (→ la prova ③), e l'app che
comincia a spingere un tipo inventato (→ le prove ③ e ④).
...
nessuno. Sulla firma curata: `payment` presente, la cura della **105** (la riga viva) intatta, il
permesso `cloud_sync` ancora richiesto.

⏳ **MANCA LA PROVA FISICA, e ora si sa che NON è piccola.** L'omaggio non si registra nel
gestionale: si segna **su Matchpoint**, sulla ficha, mettendo un giocatore *riscosso* a **0 €**.
Il gestionale non ha un bottone: quando la scheda partita legge il roster dal worker,
**riconosce** quello stato e lo persiste. ⇒ La prova vuole un gesto sul gestionale del circolo, su
una prenotazione vera, e nessuno l'ha mai fatto in tre mesi.
⚖️ **Quindi la voce resta qui come la 65: curata, in servizio, si aspetta il caso** — e il caso
qui non si provoca a comando senza toccare il Matchpoint vero. La sonda è pronta e oggi torna 0.
...
## 📦 CHIUSE — dal 13 al 30/08/2026 — 95 voci
