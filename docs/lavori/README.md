...
| | |
|---|---|
| **104** | 🎲 **Le opzioni delle domande di scheda escono sempre nello stesso ordine, e quell'ordine È la scala** — 🗣️ voce sua del 28/08 a mezzogiorno, con lo screenshot della domanda sul vetro davanti: *«diverse domande sono in ordine crescente di livello… dovrebbe essere un ordine random, se no dopo un po' la gente capisce questo sistema»*. ⇒ È un difetto del **cancello**, non di stile: chi rifà il test qualche volta impara che *l'ultima opzione è la più alta* e pilota il livello senza saper giocare. La banca di conoscenza ha già memoria e pescata a caso; le **otto domande di scheda** no. ✅ **Misurato che mescolare è sicuro per il punteggio**: nasce dal **testo** (`assessmentPublicScoreFromText`), non dall'indice, e `scelte()` tiene il `valore` attaccato al `testo` ⇒ spostare un'opzione si porta dietro il suo punteggio. 🚨 **Ma NON si mescola dentro `SCHEDA_DOMANDE`**: `test/motore-a-passi.test.mjs` confronta quelle otto con i `<select>` di `index.html` **opzione per opzione e nell'ordine**, e quella parità esiste perché finché la pagina è viva le domande stanno in due posti. ⇒ Si mescola **al momento di presentarle** (`domandeDelGiro`), lasciando la fonte ordinata. 🔒 E la mescolata dev'essere **ripetibile col gettone** (regola A3: chi lascia a metà ritrova le sue domande), con un **sale diverso** da `ordine-del-giro`. ⛔ **Non toccare le opzioni di CONOSCENZA**: là la risposta giusta è un **indice** e la correzione avviene sul server ripescando col gettone (B7) ⇒ mescolarle vorrebbe dire mescolare anche in correzione. E non serve: quelle non sono in ordine di livello. ⏳ **Da chiedere a LUI prima di scrivere**: **quali** delle otto mescolare. Proposta — solo le **quattro tecniche** (scambio, vetro, rete, colpi alti), che sono l'unico posto dove la posizione dice qualcosa che il testo non dice già; le due scale numeriche e soprattutto le **due domande sui sette livelli** mescolate confonderebbero chi risponde onesto, e lì il nome è scritto sul bottone. 📄 Vincoli e prove da scrivere: `docs/passaggio-consegne-20260828-mezzogiorno.md` § 4bis |
...
## 📦 CHIUSE — dal 13 al 28/08/2026 — 81 voci
