...
### **108** — 🚨🚨 UN SALVATAGGIO DI SCHEDA SOCIO PUÒ SPARIRE IN SILENZIO, e lo schermo dice «salvato»

🗣️ **Nasce da un caso suo, il 29/08 sera**: aveva cambiato il livello di Maurizio Aprea da **4 a
3,5 e poi di nuovo a 4**. Nel cloud non è arrivata **nessuna delle due**, e il gestionale non l'ha
detto. Il giorno dopo lo schermo mostrava 3,5, il cloud diceva 4, e nessuno dei due sapeva di
essere in disaccordo.

📏 **LE DUE MISURE, su PROD e non a memoria:**
· in tutto il **29/08** l'unica scrittura di un record `member` è stata il sync clienti delle
  19:30 ⇒ **zero** soci su 2817 con `payload.updatedAt` di quel giorno. La serie dei salvataggi è
  continua dal 16 al 28 agosto e si azzera lì;
· sul registro `pmo_audit_log`, dal 15 al 29 agosto, **67 spinte su 411 hanno scritto ZERO
  record** — **una su sei** — e `pmoSyncCloudRecordsNow` le dava tutte per riuscite, perché
  guardava `ok` e **non** `count`. La RPC risponde `ok:true` anche quando scarta tutto.

🚨⭐⭐ **E LA COSA CHE HA DECISO LA FORMA DELLA CURA: il ramo che ha mangiato quei due salvataggi
NON SI È POTUTO NOMINARE.** Le 67 spinte a vuoto lasciano almeno la loro riga di audit; le sue
due **non lasciano nemmeno quella**, quindi non hanno raggiunto il database. E su un browser
pulito la catena funziona: 6.256 servita, tutte le funzioni al loro posto, il record che si
costruisce con la chiave giusta (verificato sull'app viva di PROD con la console remota).
⇒ I modi di perdere quella scrittura sono almeno **quattro**, in punti diversi: la spinta che non
parte (`if (!record) return`, muta), quella rifiutata, quella che scrive zero dicendo `ok`, e
quella mai raggiunta perché una chiamata dell'interfaccia ha lanciato prima. Curarli uno per uno
avrebbe lasciato scoperto il **quinto** — che è precisamente questo caso.
📌 *Una perdita silenziosa non si cura indovinando il ramo che si rompe: si cura rendendola
rumorosa.*

⚙️ **LA CURA (v6.261 su `test-preview`), tre pezzi:**
① **il conteggio**: una spinta che scrive zero record **fallisce**, invece di passare per riuscita.
   Il confronto è con lo **zero** e non con `list.length`, di proposito: un lotto può
   legittimamente scriverne meno, perché la RPC fonde i `member` che finiscono sulla stessa riga
   viva (`distinct on (local_key)`). Zero invece non ha nessuna lettura innocente;
② **la traccia**: ogni modifica al socio lascia un segno in `localStorage` scritto **prima** di
   spingere e cancellato **solo** quando il cloud conferma di aver scritto. Qualunque strada in
   mezzo si interrompa, il segno resta — e all'**avvio** le tracce rimaste si dicono, perché un
   avviso che vive un secondo è un avviso che si perde;
③ **l'ordine**: la spinta sta subito dopo `save('giocatori', …)`, non più in fondo a
   ...
   calendario non sta più dentro un `catch (e) {}` **nudo**, che è la forma più silenziosa
   possibile di perdere un salvataggio.

⛔ **COSA LA CURA NON FA, dichiarato**: non rispinge da sola. Una ripetizione automatica manderebbe
la copia di *quel* browser sopra una riga che nel frattempo può essere più fresca — è il danno
della voce 105 con un altro nome. Si dice all'operatore che quella scheda non è salita; a
rimandarla è un gesto suo.

🧪 **Banco `test/il-salvataggio-che-non-sale.test.mjs`, 15 verdi**, e **tutte e quattro le cure
viste rosse con un sabotaggio** (conteggio tolto → 2 rosse; spinta rimessa in fondo → la sua;
`catch` nudo rimesso → la sua; traccia cancellata anche sul fallimento → la sua).
🩹 **E due prove testuali sono nate rosse per una ragione da ricordare**: cercavano nel sorgente
**coi commenti dentro**, e i commenti della cura nominano `closeMemberCard()` e `updateStats()`
...
📏 **Cadute misurando, non rileggendo:**
· ② *«un altro tipo fuori whitelist»* — dei **sette** che mancavano alla lista della RPC, l'app ne
  scrive **uno solo** (`payment`): gli altri sei (`wallet_txn`, `wallet_balance`, `assessment_email`,
  `booking_job`, `staff_edit`, `staff_cancel`) in `index.html` compaiono **solo in lettura**, e tre
  non compaiono affatto. E `payment` l'app lo scrive **unicamente** dal percorso omaggi, che esige un
  giocatore *riscosso* a **0 €** — misurato zero volte in tre mesi. ⇒ **la whitelist non può
  spiegare le 67**, e con lei cade l'ultimo pezzo che legava la 109 a questo conto;
· ① *«`local_key` vuota»* — i **12** siti `staff_booking` o filtrano (`if (!b || !b.id) return`,
  `.filter(b => b && b.id)`) o generano l'id sul posto; `pmoAddCloudRecord` rifiuta la chiave vuota;
  `pmoBuildMemberCloudRecord` e `pmoBuildAppSettingCloudRecord` tornano `null` senza chiave; e
  `pmoSyncCloudRecordsNow` esce **prima** di chiamare la RPC se il lotto è vuoto. ⇒ **nessuna strada
  nota sa costruire un record senza chiave.**

⛔ **E si è controllato anche che non fosse il database a mentire**: l'unico trigger su
`pmo_cloud_records` è il `touch updated_at` e torna `new` (non sopprime righe), la RPC è
`SECURITY DEFINER`, e le due copie sono **byte-identiche** fra i progetti. ⇒ `count: 0` significa
davvero *«di tutto il lotto non è passato niente»*, e le 411 spinte sono quasi tutte da **un record
solo** (289 su 411 ne scrivono 1) ⇒ i 67 zeri sono spinte **singole**.

⚙️ **LA CURA È SUL REGISTRO, non su un ramo indovinato** — migrazione
`20260829223000_voce108_una_spinta_a_vuoto_dice_cosa_portava.sql`, applicata **ai due database**.
Quando e **solo quando** `v_count = 0`, l'audit annota `ricevuti`, `senza_tipo`, `senza_chiave` e
`tipi` (quanti record per ciascun `record_type`). ⇒ La prossima spinta a vuoto **dice da sé** quale
delle tre regole di scarto è scattata e su cosa.
📌 *Un registro che annota il RISULTATO e non l'OGGETTO non può chiudere una diagnosi* — trappola ③
del 29/08, qui curata invece che raccontata.

⚖️ **Cosa NON cambia, dichiarato**: il comportamento della RPC è identico (stessi scarti, stesso
`count`, stessa risposta) e le spinte **riuscite** continuano a scrivere `{count: N}` **nudo** ⇒
il registro non si gonfia e nessuna sonda esistente cambia lettura. Nel registro entrano **nomi di
tipo e conteggi**, mai un payload.
🔎 **E serve dove l'avviso della 108 non arriverebbe**: `_pmoGiftSyncFromRoster` si chiude con
`catch (e) { return 0; }` e inghiottirebbe l'errore. Il registro no.

✅ **PROVATA ESEGUENDO, non leggendo**: i tre casi di scarto sono stati fatti passare **dentro la
RPC vera** su TEST — tipo fuori whitelist → `{tipi:{wallet_txn:1}, senza_chiave:0}`, chiave mancante
→ `senza_chiave:1`, tipo mancante → `senza_tipo:1, tipi:{"(senza tipo)":1}` — e la spinta **buona**
ha lasciato `{count:1}` **nudo**. La riga di prova su TEST è stata rimossa.
🔒 **Guardia**: 3 prove nuove in `test/i-tipi-che-lapp-scrive.test.mjs` (9 verdi in tutto), viste
**rosse con tre sabotaggi** — campo tolto, condizione dello zero rimossa, payload letto.
⚠️ **Sono TESTUALI e va detto**: leggono il sorgente della migrazione, non interrogano nessun
database. Dicono *«la diagnostica è scritta»*; che funzioni l'ha detto l'esecuzione su TEST.

⏳ **LA DOMANDA RESTA APERTA, ed è il punto**: la causa delle 67 **non è nota**. Questa cura non la
trova all'indietro — mette in condizione la **prossima** di spiegarsi da sé. Si chiude quando una
spinta a vuoto vera lascia la sua diagnostica e la si legge.

✅⭐⭐ **LA METÀ SU PROD È ARRIVATA — 30/08 pomeriggio, dal gesto della segreteria** (63ª sessione).
Fino a stamattina la cura era provata **su TEST** in tutt'e due i versi e su PROD **in nessuno**, e non
per distrazione: dal **28/08 alle 20:54** nessun salvataggio della segreteria era più atterrato nel
cloud ⇒ la cura girava in servizio senza aver mai visto un gesto vero.
📏 **Il gesto e la sua misura**: scheda socio di `Utente Test`, livello **3 → 3,5 → 3**, cioè **due
salvataggi di fila sullo stesso socio** — la forma identica al guasto del 29/08 su Maurizio. Le due
spinte hanno lasciato `cloud_records_upsert {count: 1}` alle **15:49:34,674** e alle **15:50:03,513**,
**nessuna a zero**; la riga è atterrata **1,74 s** dopo il salvataggio (`updatedAt` 15:50:01,773 →
`synced_at` 15:50:03,513); e al ricarico **nessun avviso giallo** ⇒ nessuna traccia rimasta, cioè il
cloud ha confermato tutt'e due.
⛔ **COSA QUESTO NON PROVA, dichiarato**: il ramo del **fallimento** su PROD. Che una spinta persa
lasci la traccia e la annunci resta provato **solo su TEST**, dove la si è potuta far fallire davvero
(utenza `readonly`, rifiuto vero di `pmoRequireStaffPermission`). Su PROD si è vista la strada buona.

📏⭐ **E LA DIAGNOSTICA HA RISPOSTO «NON ANCORA», che è diverso da «tutto a posto».** La migrazione
`voce108_una_spinta_a_vuoto_dice_cosa_portava` è viva dal **29/08 20:23:16Z**; da allora le spinte
sono **11** e quelle a vuoto **0**. L'ultima a vuoto è del **29/08 13:17** — cioè **prima** che la
diagnostica esistesse: nessuna delle 67 ha mai potuto lasciare la sua riga.
🚨 **E quello zero non dice che il difetto è sparito**, ed è la trappola dello *zero letto troppo
presto*: prima cadeva a vuoto **una spinta su sei**, e su 11 spinte lo zero capiterebbe per caso
circa **una volta su sette**. ⇒ Non è una misura che assolve, è un campione troppo corto.
⏳ **Perciò la voce RESTA APERTA**, e per la stessa cosa che diceva prima: si chiude quando una spinta
a vuoto **vera** lascia la sua diagnostica e la si legge. La causa delle 67 è ancora ignota.
...
stessa forma della voce 105 — *una cura che produce un valore che nessuno riceve è verde in ogni
banco che guardi il produttore invece del destinatario.*
...
giro del cron.
🚨⭐⭐ **E la sera del 29/08 si è scoperto che in PROD questa cura è INERTE**, per una ragione che
non è sua: marca `lastLevelUpdateAt` **in locale**, e il salvataggio della scheda socio **non
arriva al cloud** — vedi la **voce 108**, misurata quella sera. ⇒ Finché la 108 non è curata e in
servizio, il cancello della data continua a leggere il valore vecchio. La prova fisica del punto D
**non si può fare prima**, e provarci darebbe un verde che non significa niente.
...
## 📦 CHIUSE — dal 13 al 30/08/2026 — 90 voci
