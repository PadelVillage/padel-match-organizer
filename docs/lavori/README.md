...
### **125** — 💶 L'INCASSO nella scheda partita — acceso, e l'edge che chiamava NON ESISTEVA

🗣️ **Voce SUA**, 02/09/2026: *«adesso direi che puoi procedere con l'accensione dei soldi nella
scheda partita»*. Messo davanti alle strade ha scelto **solo l'incasso** — lo storno resta nella
scheda socio → tab Pagamenti, dove l'aveva appena visto funzionare.

📏 **E prima di accendere ho misurato cosa c'era dietro l'interruttore, invece di girarlo.** Quel
flag — `PMO_PAYMENTS_WRITE_ENABLED` — accende **due** cose, e solo una aveva una porta:

| pezzo | edge | c'era? |
|---|---|---|
| ↩︎ storno di riga nella scheda partita | `matchpoint-payment-void` | ✅ v1 ACTIVE (scritta il 02/09) |
| incasso Contanti · Carta · Borsellino | `matchpoint-payment-write` | ❌ **non esisteva su PROD, e non era mai esistita** |

🚨⭐⭐ **È la trappola del 02/09 una seconda volta, in due giorni.** Il sorgente stava in
`supabase/functions/_archive/`, cartella che i due workflow **saltano di proposito** (filtro
`$3 !~ /^_/`): esiste in git, si legge, compare in un `grep` — e su Supabase non arriva mai.
⇒ Accendendo il flag, tre bottoni sarebbero comparsi in segreteria e **ogni click sarebbe stato un
404**. Stavolta vista **prima** che qualcuno ci passasse, non dopo.
...
🔨 **Cosa è stato fatto** (PROD 6.274 → **TEST 6.287**):
· `matchpoint-payment-write` **fuori dall'archivio** e dentro il recinto `scrittura-al-circolo.ts`
  — **decima copia**. ⚖️ E la condizione conta: il 9/08 lui l'aveva archiviata proprio perché era
  una scrittura di denaro **fuori dal recinto**, quindi torna viva solo ora che il recinto ce l'ha.
...
· **Flag nuovo `PMO_PAYMENTS_COLLECT_ENABLED`**, e nell'app la variabile che comandava tutt'e due i
  gesti è stata **sdoppiata** (`_payCollectActive` / `_payVoidActive`): una variabile sola non sa
  dire di sì a uno e di no all'altro. ⇒ **`PMO_PAYMENTS_WRITE_ENABLED` resta `false`.**
· 🧾 **NON scrive un record `payment` ottimistico**, di proposito: la sezione Incassi somma *tutti*
  i `payment` e il sync ne porta uno autorevole entro pochi minuti ⇒ ne conterebbe **due**. ⚖️ È il
  verso opposto dello storno, dove marcare subito è lecito perché si scrive **esattamente ciò che
  il sync scriverebbe** (`voided_at`); qui la riga nuova avrebbe un `seq` che non possiamo
  conoscere. 📌 *Anticipare il sync è lecito solo quando si sa scrivere la sua stessa riga:
  altrimenti non si anticipa, si duplica.*

🩹 **E una guardia nuova ha trovato una terza cosa che nessuno cercava.** Il banco
`test/porta-che-non-ce.test.mjs` è scritto sulla **classe** — *ogni* `/functions/v1/…` che l'app
compone deve avere un sorgente deployabile — e alla prima corsa è diventato rosso su
`matchpoint-payment-void`: riscritta il 02/09 **senza togliere da `_archive/` la versione di
giugno**. Due cartelle con lo stesso nome, 157 righe contro 276, e l'archiviata era quella **senza
recinto**. ⇒ Chi fosse andato a leggere «il codice dello storno» avrebbe letto il più imprudente.
📌 *Un gemello stantio è peggio di nessuna copia: chi lo trova non ha modo di sapere che non è
quello in servizio.*

🔪 **Sabotaggi**: sei, tutti visti **rossi e poi verdi** — sorgente sparito · sorgente ricomparso in
`_archive/` · storno di riga riattaccato al flag dell'incasso · cancello che ignora il flag nuovo ·
recinto tolto dalla strada di `payment-write` · decima copia divergente di un byte.
✅ Banco **82 verdi / 0 rossi**. E il caso 9 del banco del recinto ora copre **anche
`matchpoint-payment-void`**, che ieri era entrato fra le copie ma non fra le strade: *avere il
modulo e attraversarlo sono due fatti diversi, e servono due guardie diverse.*

⛔ **APERTA per la PROVA FISICA, ed è sua**: un **incasso vero da 1 €** su una partita vera di PROD.
Serve perché niente di ciò che è verde qui sopra dice che il worker incassi davvero — le guardie
sono **testuali** e il ramo reale non l'ha ancora percorso nessuno, da giugno.
⚠️ **Su TEST non si può**: il recinto rifiuta prima del worker (`AMBIENTE_DI_PROVA`), ed è il suo
mestiere. ✅ Il **kill-switch del worker** invece non è un ostacolo: è acceso, e lo dimostra la sua
ricarica del 02/09, che passa dallo stesso interruttore ed è riuscita. Non scavalca niente.

### **129** — 👛 IL SALDO WALLET A COLPO D'OCCHIO nella riga del giocatore

🗣️ **Idea SUA**, 02/09 notte: *«sotto il nome c'è solamente l'importo. Potremmo spostare l'importo
tutto a destra e sulla sinistra mettere un'iconcina con un emoticon di un borsellino, così da capire
al volo quanto il socio ha dentro. È un colpo d'occhio molto carino per chi fa segreteria e cassa
la sera.»*

📏⭐ **La misura la rende migliore di come l'ha pensata: il saldo CE L'ABBIAMO GIÀ, ed è quello vero.**
Il worker mette `saldoCents` su **ogni** riga del roster (`server.mjs:7811`, letto dalla ficha
Matchpoint nello stesso giro che legge i giocatori), sia per chi deve pagare sia per chi ha già
pagato. Oggi l'app lo usa per **una cosa sola** — decidere se il bottone Wallet è premibile — e poi
lo butta via. ⇒ Il colpo d'occhio **non costa una chiamata in più**: costa smettere di buttarlo.
⛔ **E la fonte NON è il cloud**: `wallet_balance` è una fotografia di **40 righe** (28 con credito,
misurate il 02/09) — è il report «clienti con credito residuo», fermo all'ultimo giro. Pescare da lì
darebbe saldi vecchi e assenti per quasi tutti.

🔨 **Due scelte di disegno che non erano nella richiesta, e sono la differenza fra un colpo d'occhio
e del rumore:**
① **La pastiglia si mostra SOLO a chi ha credito.** Il wallet ce l'ha una minoranza: con
`👛 0,00 €` su tre righe su quattro l'occhio smette di vederla proprio la sera che serve. Mostrandola
solo quando c'è qualcosa, la riga che si illumina è l'unica che interessa.
② 🚨 **`saldoCents` può essere `null`** (Matchpoint non dà il dato — l'app lo dice già altrove con
*«Saldo borsellino non disponibile»*). Lì **non** si scrive `0,00 €`: sarebbe dire «non ha soldi»
quando la verità è «non lo so». 📌 *È lo stesso errore della 114 («Libero» invece di «non lo so»),
e in questo progetto è già costato due volte.*

✅ **Deciso da lui**: la pastiglia è **muta**, non cliccabile. ⚖️ Ragione: nella scheda della partita
ogni cosa cliccabile è un gesto che tocca il circolo, e un bottone innocuo in mezzo a bottoni che
scrivono su Matchpoint è un invito a sbagliare mira.

🆕 **E una seconda idea sua, nello stesso messaggio**: *«potrebbe essere carino poter cliccare sul
nome del socio e gli apre la scheda di anagrafica. Ma non so se si può fare.»*
✅ **Si può**: `switchTab` e `openMemberCard(id)` esistono già, e il socio si trova confrontando
`p.idCliente` del roster con `pmoIdMatchpoint(g)` dei `giocatori`.
🚨 **Ma ha un prezzo che va deciso prima di scriverla, non dopo**: aprire la scheda socio vuol dire
**lasciare il calendario**, e il pannello della partita può avere **modifiche non salvate** (importi
digitati, giocatori aggiunti o tolti). Un click su un nome non deve far perdere quel lavoro.
⇒ Tre strade: ⓐ il nome è cliccabile **solo se non c'è niente di modificato**; ⓑ si chiede conferma;
ⓒ la scheda socio si apre **sopra**, senza cambiare tab. ⛔ Da chiedere a lui.
⚠️ E il nome resta **muto** per chi non è in anagrafica (gli «Ospite», che nella sua schermata sono
la maggioranza delle righe): un nome cliccabile che non apre niente è peggio di un nome normale.

Non scavalca niente.



### **136** — 🔔 DUE AVVISI PER LO STESSO FATTO (e il secondo copriva un bottone)

🗣️ **Sua segnalazione dal telefono**, 03/09/2026, poche ore dopo aver dichiarato provata la 134:
*«guarda visivamente come sta venendo adesso, è molto brutto. Ci sono due avvisi, è troppo pieno
per quello che si sta verificando su Matchpoint. Se lo spostiamo da lì forse è più carino, più
pulito almeno.»* 📸 Con schermata di PROD 6.283.

🚨⭐⭐ **È UN DIFETTO DELLA 134, e la scheda della 134 diceva il contrario.** Là c'è scritto che la
riga dentro la scheda e la pastiglia *«si affiancano, non sono due copie»*: **vero quando l'esito è
fuori dal bordo, falso quando è dentro**. Nello stesso istante, per lo stesso evento, erano due
copie della stessa frase — e la seconda finiva **sopra il bottone «Annulla prenotazione»**.
...
⚖️ La frase della 134 non era una svista di scrittura: era **una previsione presa per un fatto**.
Avevo misurato il caso in cui l'esito è fuori dal bordo — dove le due cose davvero si affiancano —
e ho scritto la regola generale su **quell'unico caso**. Il caso opposto non l'avevo guardato.
📌 *Una misura su un caso non autorizza una frase su tutti i casi: la generalizzazione è
un'affermazione in più, e va misurata a parte.*

🔨 **Cura (TEST 6.297 · PROD 6.284)**: la riga dentro la scheda **vince**, perché è la più vicina
al gesto; la pastiglia compare **solo** quando quella non si vede o non c'è (risposta a una frase
scritta in chat). E quando compare deve leggersi come una cosa che sta **sopra** il pannello —
fondo pieno invece di trasparente, ombra più marcata, `z-index` più alto: sul telefono la
trasparenza la faceva sembrare parte dei bottoni.

⏳ **RESTA APERTA finché non la guarda dal telefono**, che è dov'è nata: un banco che conta le
righe non vede due avvisi sovrapposti.

📌 **E il seguito è una sua idea, che sta in una voce a parte** (vedi sotto, **137**): quel «sto
lavorando su Matchpoint» non è un fatto della **scheda**, è uno stato **globale** ⇒ va fuori, in un
posto solo.
...
## 📦 CHIUSE — dal 13/08 al 03/09/2026 — 112 voci
