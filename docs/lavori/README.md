...
| | |
|---|---|
| 🔴 **Urgenti** | **1** — resta la sola **65**, curata e in servizio, aperta perché la cura non l'ha ancora vista succedere nessuno: **si aspetta, non si provoca**. 📦 **Il 23/08 se ne sono chiuse cinque**: la **63** a cura vista sul bersaglio (previsto 12:27:19 leggendo il codice, successo 12:27:20), la **66** — traccia recuperata dal worker, ipotesi smentita da `hidden=1` —, la **74** con la regola che l'accompagna (*«quando la segreteria fa un qualsiasi tipo di operazione, le persone che sono dentro la partita devono essere avvisate»*, *«corretti fino in fondo»*, *«vale anche per una lezione»*), la **76** — promossa da lui nel pomeriggio e chiusa la sera stessa: l'avviso al socio nasceva dallo **specchio** e non dalla **conferma**, e il suo argomento non era la velocità, perché l'unico produttore dei fatti era il sync, che vive leggendo Matchpoint — e la 🆕 **77**, aperta e chiusa nella stessa sera. ⭐⭐ **La 77 è la voce da rileggere fra tutte**: la cura è stata provata, ha mandato al socio un *«Sei in campo»* **falso**, è stata **ritirata in dodici minuti**, e la causa non era sua — il **sync** leggeva le due metà della fotografia di «prima» in due momenti, con l'**upsert in mezzo**, che riportava in vita la riga che la resurrezione della voce 73 cerca fra i sepolti. La stessa corsa era **già viva** per un annullo della segreteria. Curato quello (dieci righe spostate 320 più su), la cura è tornata in servizio e alle **18:06:50** è stata vista funzionare **nel caso peggiore**: export scattato **48 secondi prima** dell'annullo — la finestra esatta della bugia — e il giro dopo ha scritto `slotPrima:81 slotDopo:81 accodati:0`. ⇒ *Una cura ritirata ha pagato la diagnosi di un difetto che non era suo* |
| 📋 **In coda** | **7** — la **68** (curata e in servizio: resta finché il primo giorno vero non è stato guardato), la **69**, la **70** e la **71** entrate nella notte del 21/08, la **72** del 22/08, la 🆕 **78** del 23/08 sera, più la **60** in sezione D (due passi su quattro vivi su TEST, in attesa che parli con Wansport). 🆕 **La 78 l'ha messa in coda LUI** con la schermata davanti (*«qui mi segnala che mancano due persone, ma invece siamo tutti nella partita»*): `/prenotazioni` elenca quattro giocatori e nella stessa riga dice «2 posti liberi», perché i **nomi** e il **conteggio** nascono da due letture diverse dello stesso slot. È la terza voce della stessa famiglia della **70** e della **71**. 🔼 **La 74 è uscita di qui il 23/08** — promossa fra le urgenti e chiusa lo stesso giorno; 📦 **la 73 è stata chiusa** lo stesso pomeriggio, a cura già vista funzionare su due annulli veri |
| 📦 **Chiuse** | **66** il 13–23/08 + ~56 dal 7/08 + ~41 fino al 6/08 |
...
| **78** | 🔢 **La scheda `/prenotazioni` conta i posti liberi da una fonte diversa da quella dei nomi — e le due si contraddicono** — 🗣️ segnalata dal committente il 23/08 sera, con la schermata: *«qui mi segnala che mancano due persone, ma invece siamo tutti nella partita»*. 📏 **I fatti misurati** (`2026-08-24 · 18:00 · Campo 3`, `idReserva 9598`): la scheda elenca **tre compagni** — Benso Marchi, Fabio Antoniazzi e Andrea Antoniazzi — che col socio fanno **quattro**, e nella stessa riga dice **«2 posti liberi»**. 🔎 **Le due cifre vengono da due posti diversi in `consumer-player-readmodel`**: `compagni` nasce da `compagniDelloSlot(liste)`, che **accumula le liste di TUTTE le righe** dello slot; `giocatori` nasce da `ordineDelloSlot(schede)`, che sceglie **una** scheda — e le schede dello slot non erano d'accordo. Al momento della schermata c'era una `booking_occupancy` di 9598 ancora **viva** con `descrizione: -Maurizio Aprea.-Benso Marchi.` (**due** nomi), sepolta dal sync solo alle 18:03:03, accanto allo `staff_booking` scritto alle 17:57:19 coi **quattro**. ⇒ 4 − 2 = **2 posti liberi**. ⚠️ **Ipotesi, non causa accertata**: va verificato quale riga `ordineDelloSlot` prende davvero e con quale criterio, prima di toccare qualsiasi cosa. 🎯 **La forma però è già nota, ed è la terza volta**: è la stessa della **70** e della **71** — *un solo messaggio costruito da due fonti che rispondono alla stessa domanda, e chi legge non ha modo di sapere che si contraddicono.* Qui però non è un valore ambiguo: sono **due letture dello stesso fatto** messe una accanto all'altra nella stessa frase, il che la rende **visibile al socio** invece che solo sbagliata. 🚨 **Perché non è cosmetica**: «2 posti liberi» su una partita piena è un invito a cercare due persone che non servono — e il socio che ci prova occupa il tempo di qualcuno o si sente dire di no. 📌 **Il sospetto da verificare per primo**, perché deciderebbe la cura: se la cifra nasce da una riga **stantia** che il sync non ha ancora sepolto, allora la fonte giusta è una sola (la più fresca, `synced_at`) e non un merge — cioè la stessa medicina della **73**, applicata alla lettura invece che alla scrittura. |
...
## 📦 CHIUSE — dal 13 al 23/08/2026 — 66 voci
