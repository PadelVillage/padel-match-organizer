...
| **162** | 🚦 **IL POLLING DEL SEMAFORO CHIEDE OGNI 4 SECONDI ANCHE A UN GESTIONALE CHE RISPONDE IN 100 — ✅ CURATA il 05/09 sera, in servizio su TEST 6.369 e PROD 6.369.** 🔨 `svcGiroDiPolling`: niente più `setInterval` — il giro successivo parte nel `finally`, **dopo** la risposta; il passo lo dà `svcProssimoPasso` (pura): più lenta del passo ⇒ raddoppia fino a 32 s, veloce ⇒ torna **subito** a 4; scheda nascosta non chiede (già così). 📏 **Provata sulla pagina viva di TEST** con la console remota: chiamate a `matchpoint-queue-status` alle 19:26:49 · :54 · 27:00 · :05 UTC, cioè **4 s più la durata della risposta** (~1,4 s), mai sovrapposte — la firma del giro sequenziale, che un timer fisso non produce. 🧪 Banco 7 casi con la regola del passo presa dal sorgente e valutata; due sabotaggi visti rossi. ⏳ **Il backoff non è stato visto sul vivo**: vuole un gestionale lento, e oggi risponde in un secondo. Si chiude quando `function_edge_logs` mostra i passi allungarsi durante una lentezza vera. 🆕 *Entrata il 05/09/2026 pomeriggio, nata da una misura dell'avaria (voce 160).* 📏 Alle 06:30 del 05/09, con PROD in ginocchio, `svcPollQueueStatus` ha prodotto **129 chiamate in 10 minuti** dalle sole pagine della console remota — tre schede aperte, ognuna che chiede ogni 4 s, con risposte da **100 s** ⇒ **23 chiamate sovrapposte** in volo sullo stesso database che stava già morendo. 🚨 È la forma peggiore di carico: **cresce proprio quando il sistema rallenta**, perché il timer non aspetta la risposta precedente. 🔨 **Cosa serve**: ① il giro successivo parte **dopo** che il precedente ha risposto, mai in parallelo con lui; ② un **backoff** — se la risposta ha impiegato più del passo, il passo si allunga (4 → 8 → 16 s, con un tetto), e torna a 4 quando le risposte tornano veloci; ③ ⭐ **una scheda che non è visibile non chiede** (`document.hidden`): le tre console della mattina erano tutte in secondo piano. ⚖️ **Cosa NON deve cambiare**: quando il gestionale sta bene, la cella si accende entro i 4 s di oggi — il backoff parte solo dalla lentezza misurata, non da una soglia decisa a tavolino. 🧪 Si prova con un banco che finge risposte lente e conta quante chiamate partono; sul vivo si guarda `function_edge_logs` di `matchpoint-queue-status` (`56ee44ff…`) prima e dopo. 📌 *Un polling a passo fisso è un moltiplicatore del guasto: la cura non è chiedere meno, è chiedere in proporzione a quanto l'altro riesce a rispondere.* |
...
## 📦 CHIUSE — dal 13/08 al 06/09/2026 — 144 voci
