...
🆕 **NATA E LAVORATA IL 27/08/2026 tarda sera, e la voce la vuole la SESSIONE — dichiarato come
impone la delega.** 🗣️ Il passaggio di consegne di quella sera la lasciava fuori apposta: *«non è
ancora una voce in lista»*, e lui ha risposto **«quella voce la devi volere tu. FALLO SUBITO»**.
⚖️ **Cosa scavalca**: niente. Non passa davanti a nessuna delle nove — è entrata sotto, ed è stata
fatta subito perché **cresceva da sola**: ogni cura nuova su `main` la allargava, e il costo del
riallineo sale col tempo che passa, non col lavoro che si fa.

**Cos'è arrivato su TEST** (v6.252): il motore a passi (`passi.js`, voce 97) · il tetto e la scala
in `giro-del-test.ts`, con le **tre copie** tornate identiche byte per byte · la retrocessione
tolta · il fatto dietro il «mi tengo il livello» · il messaggio a chi sta sopra il tetto · la
**lista del maestro** in Anagrafica soci col criterio allargato della **voce 100** · il riquadro
del maestro nella scheda socio · il gettone vecchio che non si butta.

📏 **Le due cure che TEST aveva già** — le 720 opzioni corte e la domanda 4 — **non** sono state
riportate: il merge a tre vie le ha riconosciute presenti, e i conteggi dei simboli combaciano con
`main` uno per uno. 📌 *Un riallineo che raddoppia una cura non è un riallineo: è un guasto nuovo
col nome di una manutenzione.*

📏 **La colonna `progress` sul database di TEST C'ERA GIÀ** — letta su `information_schema`, non
dedotta dal fatto che la migrazione fosse in git. Al ramo mancava solo il file.

🚨⭐⭐ **E IL RIALLINEO HA TROVATO UN ROSSO CHE STAVA SU `main` DA IERI SERA.** I sabotaggi ⑧ e ⑨
della voce 61 cercavano righe che la cura del «mi tengo il livello» aveva **riscritto**: non
atterravano più, e il banco lo diceva. 📏 Misurato su rami **puliti**, in due worktree separati
come vuole la trappola ⑤ del 27/08 — `main` **2 problemi su 9**, `test-preview` **0 su 10**. ⇒ Il
rosso era di `main`; portare la catena senza toccarlo l'avrebbe portato quaggiù. **Rovesciati sul
codice vero**, non cancellati (`decision=ROSSO` per tutti e due, 9 su 9), e la stessa cura spinta
su `main` nella gemella.
📌 È la trappola ② del 27/08 **vista dal verso opposto**: *una guardia che esige una frase è la
cosa che la tiene in vita* — solo che qui la frase era già stata tolta, e la guardia è rimasta a
chiederla a un codice che non ce l'ha più.

🚨 **E UN SECONDO ROSSO, che nessuno stava leggendo: `guard-docs-truth` FALLISCE da ieri sera.**
📏 Misurato nei log di Actions, non supposto: la tabella in cima a questo file dichiarava **8**
urgenti mentre la sezione ne conteneva **9** — rossa sul #1119, sul #1117 e su ogni giro dopo.
⇒ Corretta qui (10, con questa voce). ⚖️ *Una guardia rossa che nessuno guarda è una guardia che
non c'è*: è la stessa ragione per cui `guard-worker-sync` è stata resa paziente il 14/08.

✅ **COSA È PROVATO, e dove è stato guardato** — sull'app **viva** di TEST, con la console remota:
· `app-meta.json` dichiara lo sha del merge, **17 secondi** dopo il commit, e `test.padelvillage.club`
  serve **APP_VERSION 6.252**;
· la pagina si apre e fa login: il filtro **«Da certificare dal maestro»** c'è nel menu di Anagrafica
  soci, e `assessmentAspettaIlMaestro` e `refreshAssessmentDataForMembersList` sono funzioni vive;
· il **criterio della voce 100 è stato esercitato dentro la pagina in servizio**, su tre schede
...
## 📦 CHIUSE — dal 13 al 29/08/2026 — 84 voci
