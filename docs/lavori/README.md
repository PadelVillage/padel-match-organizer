...
### **105** — 🚨🚨 UNA MODIFICA DALLA SEGRETERIA PUÒ SDOPPIARE UN SOCIO: il gestionale **calcola** la chiave invece di usare quella della riga viva

🆕🔼 **Entrata la sera del 28/08, e sta fra le urgenti.** Nasce da un **guasto vero visto succedere**,
non da un'idea: alle 22:17 una modifica di livello dalla segreteria ha sdoppiato **Maurizio Aprea**, e
due minuti dopo il bot gli ha risposto *«preferisco non scegliere al posto tuo»*.
⚖️ **Non scavalca nessuna delle sette**: quelle restano dove sono. Sta in cima perché è l'unica che si
**riattiva da sola a ogni gesto normale della segreteria**, e perché intanto lascia il socio **fuori
dal test** — `consumer-assessment-link` ha la stessa difesa e rifiuta uguale.

📏 **COM'È STATA TROVATA, ed è la parte che vale**: nessuno la cercava. È saltata fuori perché **E1**,
messa in servizio la sera stessa, ha allargato la lettura del socio a tutte e tre le scelte — quel
rifiuto esisteva già ma toccava **solo «scendo»**. Prima di quella riga il tocco sarebbe **passato**,
registrando una scelta su un'anagrafica ambigua senza che niente diventasse rosso.
📌 *Un difetto silenzioso non si trova cercandolo: si trova quando qualcosa comincia a guardare dove
prima nessuno guardava.*

🔎 **LA CAUSA, isolata fino alla riga.** `pmoMemberCloudLocalKey` (`index.html`) **calcola** la chiave
di scrittura dal telefono:

```
if (phoneDigits.length >= 8) return `phone:${phoneDigits}`;
```

⇒ Il telefono vince **sempre**, anche quando la riga viva di quel socio sta su `email:` o `name:`. La
scrittura non aggiorna la riga viva: ne **crea un'altra** — e nel caso di Maurizio ne ha **rianimata
una archiviata dal 19 luglio**, riportando `deleted` da `true` a `false`.
🚨 Una scrittura non deve poter **inventare** un'identità, e men che meno **resuscitare** una riga che
qualcuno aveva archiviato.

🩹⭐⭐ **E CORREGGE IL PUNTO C① DEL PASSAGGIO DI CONSEGNE, che cercava il colpevole altrove.** Lì stava
scritto che il buco era nella rete del sync (`eChiaveVecchiaDellaStessaScheda`, *«avrebbe dovuto
scattare»*). **Non era il suo turno**: il doppione non l'ha fatto il sync.
📏 La misura che lo dimostra, e sono due righe di SQL — le righe `member` toccate per minuto:
`17:31 → 1096` (giro di massa, il sync) · `20:08 → 1` · `20:17 → 1` · `20:21 → 1`. Le tre da **una
riga sola** sono i due gesti della segreteria e la mia archiviazione. ⇒ Scrittura **singola**
dell'app, non import. La riga vecchia si corregge, non si affianca.

📏 **QUANTO È GRANDE, misurato su PROD**: **25 soci su 2817** hanno il telefono ma la riga viva su una
chiave diversa da `phone:` — **21** su `email:`, **1** su `name:`, **3** legacy. Ognuno di loro si
sdoppia alla prossima modifica dalla segreteria. Maurizio era uno; **Fabiola Limuti è un altro**, ed
era già nominata al punto D del passaggio di consegne.

🔨 **CHE FORMA HA LA CURA** — disegno, non ancora codice: chi scrive deve scrivere **sulla riga che è
viva**, ritrovandola per `payload.id`, invece di calcolarsi una chiave. Il parametro `fallbackKey`
esiste già ma è usato al rovescio: la chiave calcolata vince su tutto. ⚠️ Oggi l'oggetto socio in
memoria **non porta** la `local_key` della sua riga — le due letture (righe 12905 e 12953) la
scartano — quindi la cura tocca **il caricamento e i 7 punti di scrittura**, non una funzione sola.
...
Maurizio **una riga sola**, doppioni nel gestionale **0**. ⚠️ Il rattoppo non impedisce il prossimo:
è la stessa modifica dalla segreteria a rifarlo, su chiunque dei 25.

🔎 **DA GUARDARE INSIEME A QUESTA, misurato la sera stessa e non promosso a voce**: il cambio di
livello a mano **non aggiorna `lastLevelUpdateAt`** — è rimasto quello dell'applicazione automatica.
Quel campo regge la protezione *«una scheda vecchia non deve scavalcare un livello aggiornato dopo»*
(`assessment-apply-level`). Se la segreteria abbassa qualcuno a mano e resta in giro una scheda più
recente di quel timestamp, il giro può **rialzarlo**. Stasera non ha fatto danno; non è stato
misurato quanto sia raggiungibile.
