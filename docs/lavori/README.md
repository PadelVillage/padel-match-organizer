...
🔎 **Trovata tirando il filo della 108, ed è la CAUSA delle sue 67 spinte a vuoto.** La 108 le ha
rese rumorose; questa dice **perché** succedevano.
...
🚨⭐⭐ **IL DANNO, MISURATO E NON SUPPOSTO:** nel cloud ci sono **3032** record `payment`, e di
questi **ZERO** vengono dall'app (`source = 'pmo_gift'` → 0, `method = 'gift'` → 0). I 3032
arrivano tutti dal sync dei pagamenti, che usa la chiave di servizio e questa RPC **non
l'attraversa**.
⇒ **Ogni omaggio che la segreteria ha registrato dalla scheda partita è andato perduto.**
⚠️ E si vedeva benissimo: `_pmoGiftSyncFromRoster` chiama `_staffCalPaidIndexAdd` che scrive
l'indice locale *«senza aspettare la sync»* ⇒ l'omaggio **compariva subito** sullo schermo e
spariva al primo ricaricamento, perché quell'indice si ricostruisce dal cloud.
📌 *Il difetto peggiore non è quello che non funziona: è quello che funziona finché guardi.*
...
⏳ **MANCA LA PROVA FISICA**, ed è piccola: la segreteria registra un **omaggio** su una partita
di TEST, e la riga compare nel cloud con `source: 'pmo_gift'`. Oggi quella sonda torna **0**; dopo
deve tornare **1**. Finché non è successo, la voce resta qui.
