...
📌 **E TRE DECISIONI, che non sono prove**: la **111** (la regola sull'età — un cancello sull'età vuole
...
risposta misurata: **in parte no**, e il buco è preciso. Poi: *«mettila in lista e falla perché ci
servirà appena abbiamo finito di fixare il gestionale»*, e una riga che ha cambiato il disegno:
*«il nostro chatbot funziona sia a gesti che a parole»*.

📏 **IL BUCO, misurato e non supposto** *(repo `assistente-padel-agent`, 04/09)*: dal cloud arrivo
al **banco** (122 file di prova), al **deploy** (`deploy-bot-hetzner.yml`), al **registro dal vivo**
(`stato-bot.yml`) e al **ponte** (le 4 edge `consumer-*`). Quello che **non** posso fare è
**mandare un messaggio al bot**: per scrivere a un bot serve un **account Telegram** — un bot non
può scrivere a un altro bot, e l'API dei bot non ha modo di iniettare un messaggio *in arrivo*. E
il nostro è in **long polling**, non a webhook ⇒ non c'è nemmeno una porta dove posare un update
finto. ⇒ Il primo anello (il dito che scrive) e l'ultimo (il messaggio sul telefono) restano suoi.
📌 *Sul gestionale arrivo al **risultato**; sul bot arrivo alla **causa** e all'**effetto** — posso
far succedere la cosa e leggere nel registro che è successa — ma non posso **essere il socio**.*

⭐⭐ **LA STRADA C'È, e l'ha aperta una misura del codice, non un'idea**: il ciclo del polling
(`bot.ts`, `avviaBot`) fa **una cosa sola** con quello che riceve — `gestisciMessaggio(u.message)`
oppure `gestisciTocco(u.callback_query)`. **Due porte, e nessun'altra.** ⇒ Una conversazione si può
dare in pasto al bot **senza Telegram**, chiamando quelle due, e senza toccare una riga di ciò che
gira: il collaudo aggiunge un ingresso, non cambia un comportamento.
⭐ **E la seconda misura è quella che rende leggibile il risultato**: *tutte* le chiamate a Telegram
passano da **una sola** funzione (`api.ts`, `chiamata()` → `fetch` su `api.telegram.org`). ⇒
Intercettando il `fetch` verso quell'host si raccoglie **l'intera trascrizione** di quello che il
bot avrebbe detto — testo, bottoni, riscontri ai tocchi — mentre le chiamate al **gestionale**
passano vere. *La finzione sta sul filo di Telegram e basta: dal ponte in giù è tutto reale.*

🎯 **COSA DEVE SAPER FARE, e i gesti non sono un extra** — sua riga esplicita:
· **parole**: un messaggio di testo libero, come lo scriverebbe un socio;
· **gesti**: il tocco su un bottone, e **indicato per l'etichetta che si legge** («premi *Ci sto*»),
  non per il codice interno — perché è quello che fa un socio, e perché un copione scritto coi
  `callback_data` diventerebbe falso al primo cambio di formato e **resterebbe verde**;
· la **trascrizione** in chiaro: cosa ha detto il bot, quali bottoni ha messo, in che ordine.

🔒 **LE SERRATURE NON SI RISCRIVONO, si eredita quelle che ci sono.** Il collaudo entra come un
**terzo modo di `avvio-prova.ts`** (accanto a `--solo-controlli`), **dopo** le sue cinque verifiche:
token del bot di prova · non è quello dei soci · i ponti guardano l'ambiente giusto · le scritture
al circolo bloccate · le nostre righe in `test`. ⛔ Un file gemello che le ricopiasse sarebbe la
trappola dichiarata di questo progetto: due copie della stessa serratura divergono, e diverge
sempre quella che nessuno guarda.
🚨 **E una serratura nuova, che si DIMOSTRA invece di crederci**: il collaudo **non chiama mai**
`getUpdates` — l'intercettore lo fa **fallire di proposito**, così «non fa polling» non è una
promessa nel commento ma un fatto che il codice non può violare. È la difesa dal **409**: due
lettori sullo stesso token si rubano i messaggi dei soci.

⏳ **COSA QUESTA PROVA NON DIRÀ, e va scritto prima di scriverla**, o si chiuderà una voce con una
...
spazio. E le colonne della scheda sono `330px 423px`: la sinistra è **fissa**, quindi su uno
...
· il **pannello** smette di essere un numero e diventa un intervallo — `clamp(min, %, max)` — così
  cresce col monitor invece di restare fermo;
· le **colonne della scheda** diventano proporzionali (`fr`) invece di `330px` fisso;
· le **tredici soglie** si guardano insieme: quante servono davvero, e quali sono la stessa domanda
  scritta due volte.
...
🗣️ **Voce SUA**, il 04/09 sera, guardando la scheda del 7/09 che mi aveva appena autorizzato:
*«a livello grafico puoi migliorarla così da far entrare tutte le informazioni dentro la scheda
aperta»* — e poi, sulla variante da mettere nello spazio che si libera: **«fai la A»**.

📏 **MISURATO su PROD 6.338 a 1440×900 prima di disegnare**, sulla sua partita: la finestra bianca
è **860 px**, la scheda ne usava **612** — **231 vuoti a destra** — e intanto **453 px** di
contenuto finivano sotto il bordo: per arrivare a «Salva» si scorreva.
⇒ *Lo spazio c'era, ed era dalla parte sbagliata.*

🎨 **Mockup approvato**: `mockup/scheda-entra-tutta-mockup.html` — e si **misura da sé**, invece di
dichiarare: a ogni variante rilegge l'altezza vera del disegno e scrive se entra nei 761 px. È
così che si è scoperto che la variante **B** sforava di **5 px**, prima di scriverla.
📌 *Una proposta grafica che dichiara «entra» senza misurarsi ha la forma di una verifica e non
verifica niente — è il banco che ricopia la regola, un piano più in là.*

🔨 **① DUE COLONNE da 900 px in su**: a sinistra Slot · Note · bottoni · il conto, a destra
Giocatori e pagamenti. Sotto i 900 resta **una colonna sola**, e la soglia è **quella che l'app usa
già** (`svcAutoCloseIfMobile`): due soglie per la stessa domanda divergono al primo cambio.

🚨⭐⭐ **SOLO CSS, IL DOM NON SI TOCCA — e non è pigrizia, è la 150.** La strada ovvia era incartare
le sezioni in due `<div>` colonna. ⛔ Ma `_svcSchedaEsito` inserisce la riga dell'esito come
**figlio diretto** del box, e la guardia della 150 la cerca con `.svc-edit-box > .svc-edit-esito`:
due involucri in mezzo e **tutte e due smettono di trovarla**, senza che niente si rompa a voce
alta — il doppione dei messaggi tornerebbe e il banco della 150 resterebbe **verde**.
📌 *Una disposizione si cambia con la disposizione. Chi cambia la PARENTELA per ottenere un layout
paga in guardie che muoiono in silenzio.* ⇒ Il banco ha una prova apposta che lo vieta.

🔨 **② IL CONTO DELLA PARTITA**: *a carico · già incassato · manca*. Regola **PURA**
(`_pmoContoPartita`), così il banco la esegue invece di ricopiarla.
🚨⭐⭐ **E la parte che vale più dei tre numeri è `ignoti`**: se anche **una** riga ha l'importo non
letto, quella somma non è il conto della partita — è un **minimo**, e la scheda lo scrive in
chiaro. È la **149 un piano più su**: là una casella diceva `0,00` per «non lo so», qui sarebbe un
**totale** che dice `24,00` per «24,00 più qualcosa».
📌 *Un totale che ha saltato una riga non è approssimato: è falso, e guardandolo non si distingue
da uno giusto.*

🩹⭐ **E IL VECCHIO «Totale da incassare» È STATO TOLTO, non affiancato.** Rispondeva a **una** delle
tre domande e il conto la contiene: tenerli tutti e due sarebbe stato il **quinto** giro dello
stesso difetto — 136 · 145 · 147 · 150, *due oggetti che dicono lo stesso fatto* — stavolta **sui
soldi**. ⚖️ E aveva un difetto che il conto non eredita: sommava con `c > 0`, quindi una quota
**offerta** (0,00 letto) e una **non letta** finivano nello stesso silenzio.

🧪 **BANCO**: `test/il-conto-della-partita.test.mjs` — **12 prove** che *eseguono* la regola, più il
freno che protegge la 150. **Sabotato nove volte: nove rossi.**
🩹 **E due rossi del primo giro erano della SONDA, non dell'app**: una prova cercava due
`setAttribute` «entro 600 caratteri» (ce ne sono centinaia in mezzo), un'altra cercava «Totale da
incassare» nel sorgente intero e la trovava **nei commenti che spiegano perché è stata tolta**.
📌 *Una sonda tarata su una DISTANZA misura quanto è lungo il codice; una che cerca una parola non
distingue il codice dal racconto del codice.*

✅ **PROVATO sulla pagina viva di TEST 6.340**, alle due larghezze:
· **1440×900** → `grid-template-columns: 330px 484px`, i giocatori a `x=342`, **contenuto 744 in
  744 visibili ⇒ non si scorre più**. I 453 px di scorrimento sono diventati **zero**;
· **390×844** → `grid-template-columns: none`, tutto a `x=0`: una colonna sola, come prima.
· Il conto, su una partita saldata: *A carico 48,00 · Già incassato 48,00 · Manca 0,00*, senza
  avviso di parzialità — che è giusto, lì non mancava niente.

⏳ **PERCHÉ RESTA APERTA**: ⚠️ il caso **«conto parziale»** (una riga con l'importo non letto) sulla
pagina viva **non è ancora passato** — la partita di prova era pagata per intero, quindi quel ramo
l'ha eseguito solo il banco. E manca il suo occhio su PROD.

### **151** — 💶 L'IMPORTO SI RICORDA, E DICE QUANDO — 🔨 **CURATA, aspetta il suo occhio**

🗣️ **Voce SUA**, il 04/09: *«sull'importo c'è un trattino, dopo circa otto secondi appare
l'importo. Ma questo non è normale perché ci siamo detti che dovrebbe apparire già l'importo visto
che ogni due minuti andiamo a leggere Matchpoint attraverso il worker.»*
🗣️ **E la sua approvazione, esplicita**: *«Sì, procedi col salvare l'importo quando si legge.»*

🚨⭐⭐ **LA PREMESSA DELLA SUA FRASE È FALSA, ed è la misura che ha deciso tutta la strada.** Il
sync ogni due minuti **non legge gli importi**: in `matchpoint-bookings-sync` il tipo è
`giocatori?: string[]` — **solo nomi** — perché la fonte è la **descrizione** dell'export Excel.
Gli importi stanno nella **ficha** della singola prenotazione, che il sync non apre mai.
📏 E aprirle tutte non è una strada: **206** prenotazioni × un giro ogni 2 minuti sul worker
**condiviso con PROD** ≈ **150.000 visite al giorno**.
⇒ L'unica lettura che vede gli importi è quella che parte **aprendo la scheda** — ed è la ragione
dei suoi otto secondi. 📌 *Quando qualcuno dice «ma questo dovrebbe già esserci perché facciamo
X», la prima cosa da misurare è se X fa davvero quello che tutti credono.*

🔨 **LA CURA: la lettura non si butta più.** `_staffCalPersistRosterFromWorker` teneva i **soli
nomi** — era scritto anche nella scheda della **132**, che l'aveva già misurato: *«un importo messo
là sarebbe stato cancellato dalla lettura successiva»*. Adesso quella funzione tiene anche i soldi,
e li spinge al cloud come già faceva coi nomi.
⭐ **Il write-back esisteva già**: non è stata costruita nessuna strada nuova, è stata smessa una
buttata via. E regge perché **il sync non riscrive il payload delle `staff_booking` attive** — le
tocca solo per cancellarle ⇒ un importo salvato lì **sopravvive** ai giri del sync.

🚨⭐⭐ **E IL PEZZO CHE RENDE LA CURA ONESTA: `lettoAt`.** Un importo ricordato è un importo
**vecchio**, e senza il *quando* un numero di quaranta minuti fa e uno appena letto sono **la stessa
cosa sullo schermo**. ⇒ La casella guadagna un **terzo esito** — `ricordato` — accanto a *letto* e
*non letto*:

| | la casella | il «da» del cambio importo |
|---|---|---|
| **non letto** | vuota, segnaposto `—` | `(non letto) → 9,00` |
| **ricordato** | il numero, bordo **tratteggiato** | `8,00 (letto alle 21:03) → 9,00` |
| **letto adesso** | il numero, bordo pieno | `8,00 → 9,00` |

⚠️ **Senza quel terzo esito la 151 avrebbe ROTTO la 149 al contrario.** La 149 ha stabilito che un
«da» che nessuno ha letto non si inventa a 0,00. Un numero **vecchio** stampato come se fosse appena
letto è un «da» inventato **esattamente quanto lo zero** — solo più credibile, perché è un numero
vero: *è stato vero, prima*. 📌 *Una cura che riempie un vuoto deve dire con che cosa lo ha
riempito, o il vuoto era più onesto.*

⛔⛔ **QUELLO CHE NON SI RICORDA VALE PIÙ DI QUELLO CHE SI RICORDA:**
· **il BORSELLINO (`saldoCents`) no**: non è un fatto di *questa* prenotazione — è quanto ha in
  tasca una persona, e si muove per gesti che con questa partita non c'entrano. Sarebbe **falso per
  cause esterne**, e nessuno lo saprebbe. La pastiglia della 139 continua a dire «—» finché non ha
  il dato vivo;
· 🚨⭐⭐ **lo STATO (riscosso / in sospeso) no, ed è la scelta che costa di più spiegare.**
  Ricordarlo renderebbe la riga *più stabile* — niente numero che si trasforma in pastiglia dopo
  otto secondi — ma il verso in cui può sbagliare è quello caro: un **«✓ pagato» ricordato**, su un
  pagamento **stornato** nel frattempo, fa **saltare un incasso vero**. ⚖️ *Un importo vecchio si
  corregge da sé alla prima rilettura; un incasso non fatto no.*
  ⇒ Senza `stato` la riga resta **modificabile** per quei secondi, e non è un buco: chi provasse a
  cambiare l'importo di una riga già riscossa si sente rispondere **`ALREADY_PAID`** dal worker
  (voce 132). *Dove la protezione esiste già a valle, a monte si può scegliere di non sapere.*

🚨 **E LA GUARDIA «È CAMBIATO?» ORA GUARDA ANCHE I SOLDI.** Confrontava i **soli nomi** e usciva
dicendo «già allineato»: con gli importi nel record, un cambio importo su Matchpoint (stessi
giocatori) **non sarebbe mai stato ricordato**. 📌 *Una guardia «è cambiato?» che confronta meno di
quello che salva non protegge: congela.*
⛔ `lettoAt` resta **fuori** dal confronto, di proposito: cambia a **ogni** lettura, e includerlo
riscriverebbe il record e ripingerebbe il cloud ogni volta che qualcuno apre una scheda — *un
campanello che suona sempre non dice più niente.*

🧪 **BANCO**: `test/limporto-si-ricorda-e-dice-quando.test.mjs` — **27 prove** che *eseguono* le
regole (la casella, il «da», che cosa si ricorda, la guardia, e il write-back **end-to-end** con
`safeLoad`/`save` finti). **Sabotato dodici volte: dodici rossi.**

🩹⭐⭐ **E UN SABOTAGGIO SBAGLIATO HA SCOPERTO UN BUCO VERO NEL BANCO.** Togliendo dalla chiave del
confronto il solo `pendenteCents`, il banco restava **verde**: la prova cambiava i due numeri
**insieme**, e a riconoscerlo bastava l'altro ⇒ difendeva *«almeno uno dei due»*, non *«tutti e
due»*. Aggiunte le due prove che mancavano (solo il pendente · solo l'importo), e adesso mordono.
📌 *Un sabotaggio che non fa diventare rosso non dice «la cura è solida»: dice che il banco guardava
altrove — e va creduto lui, non il verde.*

🩹⭐ **IL BANCO DELLA 149 È STATO AGGIORNATO E RAFFORZATO, non allentato.** La sua prova ⑤ cercava
**a parole** una stringa che la 151 ha sostituito con la funzione `_pmoImportoDa` ⇒ è diventata
rossa. ⚖️ **Rossa, non cieca**, ed è la differenza che conta: la guardia della **147** poggiava
sull'*esistenza* di un pezzo e quando quel pezzo è stato tolto è morta **in silenzio**. Ora la prova
① controlla che i due punti **deleghino** alla regola e ② la **esegue** sui tre esiti — cosa che
prima nessuno faceva.
🩹 **E la sua ④ era tarata su una FINESTRA a caso** (`APP.slice(i, i + 4200)`): il ramo è cresciuto
di poche righe e `amt.disabled` è finito **fuori** ⇒ il banco dichiarava disabilitata una casella che
nessuno aveva toccato. Ora il ramo si chiude **contando le graffe**. 📌 *Una sonda tarata su una
LUNGHEZZA misura quanto è lungo il codice, non cosa fa: cresce il codice e la sonda mente, senza che
nessuno abbia rotto niente.*

⏳ **PERCHÉ RESTA APERTA — e cosa manca esattamente**: il suo occhio su una scheda vera, ad aprirla
e vedere **l'importo già lì** invece del trattino. 📏 Provato sulla pagina viva di TEST con un record
**seminato** a mano (la metà che si legge); la metà che **scrive** — la lettura del worker che
atterra e persiste — dalla console non si attraversa, perché `/functions/v1/` è chiuso in sola
lettura. ⇒ Il banco la esegue end-to-end, la pagina viva no.

### **150** — 🔔 L'ESITO DETTO DUE VOLTE IN FONDO ALLA SCHEDA — 🔨 **CURATA, aspetta il suo occhio**

🗣️ **Voce SUA**, la sera del 04/09 dopo un Salva su **PROD 6.334**: *«Continuano ad esserci due
messaggi uno dopo l'altro in basso alla scheda, che ora si chiude dopo che ho salvato, e questo te
lo confermo.»* ⇒ Due fatti in una frase: la **148 è chiusa** (il foglio si chiude) e il doppione
**no**.

📏 **MISURATO PRIMA DI TOCCARE UNA RIGA**, sulla pagina viva di TEST a **390×844**, non dedotto dal
sorgente: nello stesso istante `.svc-edit-esito` occupava **573→650** e `#svcQueueStatus`
**771→844**, e tutte e due stavano dentro **`#svcChatPanel`** (classe `svc-semaforo-in-scheda`).
Due riquadri, uno sotto l'altro, in fondo al foglio: la sua schermata, riprodotta.

🚨⭐⭐ **PERCHÉ LE QUATTRO CURE PRIMA NON L'AVEVANO PRESO — e non è che fossero sbagliate:
governavano tutte l'ATTESA.**

| voce | cosa spegneva | quando |
|---|---|---|
| **136** | la pastiglia, contro la riga nella scheda | gesto in volo |
| **137 ⑤** | gli avanzamenti dentro la scheda | gesto in volo |
| **145** | la riga nella scheda, sui `wait` | gesto in volo |
| **147** | la pastiglia, su un gesto in volo | gesto in volo |

⇒ Durante l'attesa parla **una voce sola**, ed è la barra: giusto, ed è la 137 ⑤. Ma **all'ESITO**
la riga torna — è voluto, è la **134** — e la striscia passa a `fatto`/`rifiutato` senza sapere che
quel fatto è già scritto **dieci pixel più su**.
📌 *Quattro cure che chiudono lo stesso difetto in quattro istanti diversi non lo chiudono: lo
spostano nell'istante che nessuna delle quattro guardava.*

🔨 **LA CURA — la regola della 136 applicata all'oggetto che le mancava.** Dentro la scheda la
**riga vince**, perché sta accanto al gesto; la striscia resta la casa dell'avanzamento (137 ⑤) e
torna a parlare appena la scheda si chiude, perché `svcCloseChat` **ridisegna già**. La regola è una
funzione **PURA** — `svcBarraDoppiaDellaRiga` — così il banco la **esegue** invece di ricopiarla: è
la lezione della **149**, dove un banco che riscriveva la regola restava verde su tre sabotaggi.

⛔ **I TRE FRENI, e nessuno è decorativo:**
· **solo il gesto LOCALE** — la striscia racconta anche le azioni di un **altro operatore**
  (`_svcUltimoSemaforo`), e quelle non sono mai il doppione del mio esito: zittirle vorrebbe dire
  nascondere ciò che qualcun altro sta facendo sugli stessi campi;
· **solo a gesto FINITO** — in corso la riga non c'è (145) ⇒ la barra è l'unica voce. È la 145 vista
  dall'altro verso, ed è il freno che impedisce a questa cura di rimangiarsi quella;
· **solo se la riga SI VEDE** — 🚨 ed è qui la trappola che la **147** ha pagato: *una protezione che
  poggia sull'esistenza di un altro pezzo muore in silenzio quando quel pezzo viene tolto*. Sta in
  piedi lo stesso perché il verso in cui cade è quello **sicuro**: niente riga ⇒ la striscia
  **torna**, e si vede un avviso invece di zero. **Muore parlando.**

⭐ **E L'ORDINE, che è la metà che non si vede e senza la quale la cura non curava niente**: la
striscia arriva a «finita» **PRIMA** che la riga esista, perché la accende e la spegne il gancio su
`window.fetch`, che si chiude sulla risposta mentre chi ha chiamato sta ancora aspettando il suo
`await`. ⇒ `_svcSchedaEsito` **ridisegna la striscia nei DUE versi** (quando scrive l'esito e quando
toglie la riga). Senza, la guardia guarderebbe sempre **un istante troppo presto** e il doppione
resterebbe identico.

🧪 **BANCO**: `test/lesito-si-dice-una-volta-sola.test.mjs` — **11 prove**, che *eseguono* la regola
e il disegno della striscia con un DOM finto. **Sabotato SETTE volte** (tolta la regola · ognuno dei
tre freni · l'aggancio al disegno · il ridisegno nei due versi): **sette rossi su sette**.
📏 Banco intero: **100 verdi, 0 rossi** (erano 99).

✅ **PROVA SULLA PAGINA VIVA DI TEST**, stesso scenario e stessa larghezza, prima e dopo:
**2 riquadri → 1**. E i due versi sicuri, misurati apposta: **senza** la riga la striscia parla
dentro la scheda; a **scheda chiusa** torna come pastiglia. Niente resta muto.

⏳ **PERCHÉ RESTA APERTA — e cosa manca esattamente per chiuderla**: un **Salva vero**, con un cambio
importo che scrive, guardato da lui su **PROD**. Dalla console remota non si può, e non è un limite
dell'app: 📏 **misurato** il 04/09 — l'utenza della console è `readonly` e
`pmoBlockWriteIfReadonly('modificare una prenotazione')` ferma `staffCalPlayersSave` **alla prima
riga**, prima di qualunque messaggio. ⇒ Ciò che ho misurato è lo **stato visivo** — che è ciò che
lui vede — non il percorso di scrittura.

🩹⭐⭐ **E IL DIFETTO DI METODO CHE QUESTA VOCE HA CORRETTO, ereditato dalla sessione prima**: il
passaggio di consegne dell'81ª diceva *«ho provato a riprodurlo su TEST e `st.payRows` era vuoto ⇒
il percorso non si è attraversato»*, e indicava come primo lavoro **capire perché payRows resta
vuoto**. 📏 Misurato: `payRows` **non era vuoto**. `staffCalPlayersState` è una variabile di
**closure**, non sta su `window` ⇒ da `page.evaluate()` si legge `undefined`, e chi la legge trova
il vuoto **della propria sonda**. La scheda intanto disegnava regolarmente **4 righe e 4 caselle
euro**.
📌 *È la 24ª un'altra volta — «un vuoto va attribuito alla propria sonda prima che al mondo» — e
stavolta il vuoto era stato promosso a **primo lavoro della sessione dopo**.* Una diagnosi
sbagliata scritta in un passaggio di consegne non resta lì: **detta l'ordine dei lavori di chi
viene dopo**.

### **149** — 💶 GLI IMPORTI SI APRIVANO A ZERO — 🔨 **CURATA, aspetta il suo occhio**

🗣️ **Voce SUA**, dal cellulare il 04/09: *«ho aperto la scheda della partita ma gli importi sono a
zero»* — mentre su Matchpoint quella partita porta **8,00** e **10,00**.

🚨 **LA CAUSA: gli importi non stanno nella copia locale.** Il roster che apre la scheda (voce 142)
nasce da `staffBookings` e porta i **nomi**; importo e stato pagamento arrivano dalla **rilettura
del worker**. Finché quella non atterra `importoCents` è **`null`**, e la casella lo disegnava
**`0,00`**.
⚖️ `null` vuol dire *«non l'ho letto»*; `0,00` dice *«non deve niente»*. Chi fa cassa la sera legge
la seconda e **non incassa**.

🚨⭐⭐ **ED È LA TERZA VOLTA CHE QUESTO PROGETTO SCRIVE QUESTA REGOLA**: la **114** (il calendario
oltre i 30 giorni diceva «Libero» invece di «non lo so») e la **139** (il saldo Wallet: `null` ⇒
`👛 —`, mai `0,00 €`, *«sarebbe dire "non ha soldi" quando la verità è "non lo so"»*). **Nessuna
delle due ha guardato QUESTA casella.**
📌 *Una regola applicata dove ci si è ricordati non è una regola: è un'abitudine, e le abitudini
hanno buchi. Messa in una funzione, il quarto punto la eredita senza chiederlo.*
🚨 **E la 142 l'ha resa PEGGIORE senza toccarla**: prima la rotellina copriva tutto e non si vedeva
nessun numero; da quando la scheda si apre subito, al posto del **velo onesto** c'è uno **zero
falso**. È esattamente il rischio che la 142 aveva previsto per i **NOMI** (*«una sezione vuota
sembra una risposta»*) e non per i **SOLDI**.

🔨 **La cura è una FUNZIONE PURA**, `_pmoImportoCasella`, gemella di `_pmoWalletPastiglia`:
· non letto ⇒ casella **vuota col segnaposto `—`** e un titolo che lo dice;
· ⛔ **NON disabilitata**: toglierebbe la correzione a mano proprio quando il worker non risponde,
  cioè nel caso in cui serve di più. *Non mentire non deve costare una funzione.*
· `baseCents` resta **`null`** ⇒ il riepilogo scrive «→ 9,00» e la **CONFERMA** scrive
  «**(non letto)** → 9,00». 🚨 Quello è il punto in cui una persona autorizza del **denaro**, e
  «0,00 → 9,00» le farebbe credere che quel giocatore non dovesse niente;
· ⛔ **e lo ZERO VERO resta `0,00`**: un omaggio è un importo **letto** che vale zero — cancellarlo
  perderebbe l'informazione opposta. *Curare «zero non è non lo so» cancellando lo zero è la stessa
  malattia al rovescio.*

✅⭐ **PROVA SULLA PAGINA VIVA DI TEST 6.334**, scheda vera a 390 px, **nei due versi**: con la
lettura **bloccata** le caselle sono **vuote col segnaposto `—`**; con la lettura **permessa** gli
importi arrivano (800, 800) e le caselle dicono **`8,00`**. ⇒ La cura dice «non lo so» quando non lo
sa, e il caso normale non si muove.
📏 E la stessa misura ha risposto a una domanda che **avevo girato a lui invece di misurarla**:
`rosterRefreshing: false` con gli importi assenti ⇒ **non è una finestra transitoria**, quando la
rilettura non li porta restano non letti.

🧪 Banco nuovo `zero-non-e-non-lo-so-importo` (6 casi), sabotato **otto** volte — cade tutte e otto.
🩹⭐⭐ **E LA PRIMA STESURA DEL BANCO RISCRIVEVA LA REGOLA INVECE DI ESEGUIRLA**: copiava le tre
righe dell'app, e sabotando l'app in **tre** modi diversi restava **verde tutte e tre le volte**.
📌 *Una copia della regola è peggio di una rilettura, perché ha la FORMA di un'esecuzione: si legge
come una prova e non prova niente.* È il motivo per cui la regola è stata estratta in una funzione
pura — così nel banco non c'è più niente da ricopiare.

🩹⭐⭐ **E I DUE DIFETTI DI METODO PAGATI PER ARRIVARCI**, che valgono più della cura:
· **le credenziali erano GIÀ tutte e quattro nell'ambiente**, e gliele ho chieste. Mezz'ora attorno
  a un problema che non esisteva, senza aver fatto la prova da tre secondi che lo diceva;
· **ho concluso «da qui non si può» da una misura che non lo dimostrava**: la scheda mi risultava
  vuota perché la aprivo con **parametri indovinati** (nome, durata e tipo sbagliati) e la
  prenotazione non veniva trovata. Presi quelli veri dal record, si è aperta al primo colpo.
  ⇒ Ed è da lì che è nata la domanda girata a lui. 📌 *Un vuoto va attribuito alla propria sonda
  prima che al mondo.* È la **26ª**, due volte nella stessa ora.

⏳ **PERCHÉ RESTA APERTA**: che sullo schermo si veda il trattino al posto dello zero lo dice il suo
occhio. E su **PROD** il ramo «lettura permessa» non è provato — la guardia dell'ambiente blocca
`--allow-writes` là.
...
dalla scheda, la risposta di Matchpoint la possiamo vedere secondo te da qualche altra parte, tipo
in sovrapposizione sul calendario. Che dici?»* — e poi, misurando lui stesso il vincolo:
*«ricordati che si utilizza la web app sia da mobile che da desktop»*.

🚨⭐⭐ **IL MOTORE C'È GIÀ, ED È SPENTO DA TRE MESI. Misurato il 03/09 prima di disegnare niente.**
Il dato «cosa gira su Matchpoint, chi l'ha chiesto, quanti in fila» **è già prodotto e arriva già
fino all'app**: manca **soltanto un posto dove metterlo**.

| pezzo | stato misurato |
|---|---|
| coda del worker | `mpQueuePump` fa `if (mpQueue.running) return` ⇒ **concorrenza 1** |
| edge `matchpoint-queue-status` | **ACTIVE su PROD**, v25 — torna `running` (cosa · chi · da quando), `waitingCount`, `busy` |
| `svcPollQueueStatus` · `svcRenderQueueStatus` | **scritte nell'app**, complete |
| la riga che le accende | **commentata**, e l'elemento `#svcQueueStatus` **rimosso dal DOM** |

⚖️ **E il perché di quello spegnimento cambia tutto**: commit `58fee80`, **3/06/2026, v5.627**, e
**non era un difetto del dato** — era **ingombro**. Parole del registro di allora: *«tolto il banner
azzurro nel pannello Calendario e il relativo polling. Effetto collaterale voluto: la riga di input
della chat torna allineata in alto con l'intestazione dei campi»*.
⇒ Il banner dava fastidio perché stava **dentro il flusso**. Una **sovrapposizione non ingombra**:
la ragione del 3 giugno **cade**, ed è esattamente la strada che ha intuito lui.
📌 *Un pezzo funzionante spento per una ragione di layout resta spento per sempre, perché nessuno
rilegge il motivo — ed è la 26ª un'altra volta.*

⭐ **E c'è un fatto che rende il suo disegno giusto e non solo comodo, con la sua parola**:
*«l'operatore di segreteria fa un'operazione alla volta, il che è corretto per come siamo
organizzati, ti torna?»* ⇒ **Sì, e non solo per organizzazione: per costruzione della macchina.**
La coda ha concorrenza 1, quindi anche due postazioni che cliccassero insieme verrebbero messe in
fila. «Cosa sta succedendo» è **un solo fatto** ⇒ un dato singleton **non appartiene a una scheda,
appartiene alla pagina**. È la stessa ragione per cui la 136 è nata.

⚠️ **Il puntino di oggi NON copre questo**: `_staffCalSetSyncing` scrive `syncing:true` in
**`localStorage`** ⇒ si accende **solo per chi ha cliccato, su quel dispositivo**, e solo su righe
già `staff_booking`. Un gesto avviato da un'altra postazione o dal bot **non lo vede nessuno**.

🎨 **QUATTRO DISPOSIZIONI DISEGNATE** (a larghezza telefono **e** desktop, per il suo vincolo) e
messe davanti a lui, come per la 131 — *un esempio prima di scrivere*:
· **A** pastiglia nell'angolo della griglia — non sposta niente, ma sul telefono tronca la frase;
· **B** solo la cella che pulsa — dice **dove**, ma 🚨 **se lo slot è fuori vista non dice niente**,
  e il silenzio è indistinguibile da «non sta succedendo nulla»;
· **C** barra sotto l'intestazione, nel flusso — ci sta tutto, ma **sposta la griglia di ~22px**:
  riaprirebbe il 3 giugno **e** la voce 130;
· **D** barra bassa **sovrapposta** alla griglia **+ la cella accesa** quando lo slot è in vista.
🔨 **Consigliata la D**, e dichiarato il perché: è l'unica che **non sposta la griglia** e insieme
...
## 📦 CHIUSE — dal 13/08 al 04/09/2026 — 126 voci
