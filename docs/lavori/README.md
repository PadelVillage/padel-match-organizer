...
✅ **POTATURA FATTA su TEST (6.241) e PROMOSSA A PROD (6.232) il 15/08, su conferma separata: 105 funzioni, 1590 righe su entrambi i rami.** ⭐ Su `main` il perimetro è stato **rimisurato, non ricopiato** — 102 funzioni / 1427 righe qui contro 103/1491 là, e l'insieme di `main` è risultato un **sottoinsieme stretto** di quello di TEST: nessun nome nuovo. Punto fisso raggiunto in 2 giri su entrambi, con lo stesso totale. Autorizzata da lui col perimetro rimisurato. Il numero della scheda — «64 funzioni e 1195 righe» — era sbagliato **in tutti e due i sensi**, e per trovarlo sono serviti quattro analizzatori: il filtro per prefisso `assessment*` nascondeva 34 funzioni (`buildAssessment*`, `openAssessment*`, `renderAssessment*`); la sonda delle invocazioni dinamiche non vedeva la forma `assessmentProcessButton(label, tipo, "nomeFunzione(...)")` e produceva **due falsi morti** che avrebbero rotto due bottoni vivi; e soprattutto **l'analizzatore leggeva un solo blocco `<script>` su cinque** — l'app ne ha uno da 976.000 caratteri — quindi 645 funzioni erano invisibili come chiamanti. 🎯 **A smascherarlo è stata la verifica dopo il taglio, non il taglio**: due riferimenti orfani rimasti nel file potato. Senza quel controllo avrei cancellato codice vivo con tutti i verdi accesi. Potato fino al punto fisso (morte residue: **0**), banco 93/93, zero orfani su 105 nomi, nessuna citata altrove nel repo. ✅ **Promossa a PROD il 15/08** con la conferma separata che la regola richiede (PROD 6.232), **a righe** e col perimetro rimisurato di là. ⚠️ **Quello che resta della 28**, ora con una ragione in più per non correre: dentro una
...
## 📦 CHIUSE — 13, 14 e 15/08/2026 — 26 voci
...
**Le prime otto voci sono del 15/08** — sei chiuse e **due annullate**, e l'etichetta lo dice riga per riga perché «non serviva più» e «è stato fatto» non sono la stessa cosa. **Le dieci successive sono del 14/08; le otto ultime del 13/08.**
