...
⏳ **NON È FINITO, e si dice invece di scriverlo in fondo a un «fatto»**: la cura è su
**`test-preview`** e **non è in PROD**, quindi oggi non protegge nessuno. Mancano ① la promozione a
`main` **per righe** (regola 3 del `CLAUDE.md`) e ② la **prova fisica**: la segreteria cambia un
livello a mano, e una scheda più vecchia in attesa **non** lo scavalca al giro del cron.
