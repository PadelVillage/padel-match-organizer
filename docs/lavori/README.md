...
🔄 **Aggiornamento del 15/08, 20ª sessione.** La **31** è stata censita e la sua mezza promozione curata (PROD **6.227**); la **29** è **chiusa dichiarando**, su sua delega esplicita; la **28** è misurata fino in fondo — 64 funzioni e 1195 righe, più 12 punti vivi che puntano a pannelli tolti — e **aspetta una decisione di prodotto**, non altro lavoro di misura. ⇒ Le urgenti scendono da **6 a 5**, e delle cinque rimaste **due non si fanno dal cloud** (34 e 14bis) e una (**26**) aspetta la 34.
...
| reperto | misura |
|---|---|
| ✅ **il banco di regressione** `test/handle-test.html` — **CHIUSO il 15/08 (PROD 6.230)** | `main` aveva **55 casi**, `test-preview` **90**. 🚨 **La prima conta era sbagliata nella COMPOSIZIONE**, e va detto: confrontavo per **id**, ma gli id sono **rinumerati** fra i rami ⇒ la famiglia «vai a GUARDARE su Matchpoint» sembrava mancare e invece `main` **ce l'aveva**, numerata 36–49 invece di 55/56/72–83. Rifatto il confronto per **nome** — l'unica cosa stabile: mancavano **35**, `main` sottoinsieme stretto (0 casi esclusivi), e il grosso erano i **PAGAMENTI** (18), non l'asincrono. ⇒ Portati **32**; i 3 lasciati là sono la **simulazione incassi**, che in produzione non ha senso. ⭐ 54 dei 55 condivisi erano **identici byte per byte** (l'unico scarto è un commento) ⇒ adottata la numerazione di TEST, così i due banchi tornano confrontabili per id e i rimandi interni («controllo opposto del 77») restano validi |
...
| ✅ `livello-dimostrato.ts` — **PROMOSSO il 15/08** | il livello **in prestito** non apre la porta dell'organizzare. Scritto su TEST il 9/08 in `consumer-player-readmodel` — **il ponte del bot** — e rimasto lì per sei giorni. 🔬 **Premessa rimisurata sui dati vivi di PROD prima di toccare**, non ripresa dalla nota del 9/08: 2797 soci vivi, **533** con un livello vero, **517** con origine VUOTA (identico al 9/08) e **`ereditato` = 0**. Le uniche origini esistenti sono `autovalutazione` (17) e `autovalutazione_da_verificare` (1) — nessun quasi-prestito che il modulo non riconoscerebbe. ⇒ **La promozione oggi non cambia il comportamento di nessuno**: chiude la porta *prima* che qualcuno possa passarci. 🔬 Provato coi denti: i 9 casi del modulo passano; girati sulla **riga vecchia** (`!!level && level !== '0.5'`) cadono **esattamente i 3 discriminanti** e restano verdi i 3 che proteggono i 517 a origine vuota. ⭐ È la forma NEGATIVA a essere la decisione importante: chiedere una prova a chi non poteva lasciarla avrebbe tolto il diritto di organizzare a **517 soci su 533** |
| ⬅️ `VERSIONI.md` — **allineato il 15/08** · `manifest.json` — **NON si tocca, e la ragione è una misura** | `VERSIONI.md` su TEST era fermo al 14/06 e **sottoinsieme stretto** di quello di `main` (0 righe esclusive): allinearlo non perde niente, fatto. 🚨 **Su `manifest.json` avevo scritto il falso**: dicevo *«l'app di prova si installa come Padel Village»*. Non è vero — `https://test.padelvillage.club/manifest.json` risponde **404** (`text/html`, la pagina d'errore di Pages), mentre su PROD è **200 `application/json`**. Il caricatore di TEST è un repo a parte che contiene **solo** `index.html`, `config-test.js` e `CNAME`, e il link nell'app è **assoluto** (`/manifest.json`) ⇒ si risolve sulla radice del dominio, dove quel file non esiste. **Il `manifest.json` di `test-preview` non lo serve nessuno.** ⇒ Sistemarne il contenuto lì non sistema niente; se un domani si vuole un manifest per TEST, va messo **nel repo del caricatore**, non qui — ed è una decisione di nome, non una potatura. 📌 Trovato di striscio: il `<title>` statico dice **v5.597** su tutti e due i rami. Lo riscrive l'app all'avvio (`APP_VERSION` + ` TEST`), quindi non si vede — ma è il ripiego che il browser usa **proprio quando il manifest manca**, cioè su TEST |
...
✅ **Fatto il 15/08 (PROD 6.230):** i 32 casi sono su `main`, che passa **87/87**. Entrano anche i **5 agganci** che chiamano — e **non** i due della simulazione. 🔬 **Provati coi denti**: sabotando il payload verso `matchpoint-wallet-correct` la rete va **85/87** e cadono esattamente i due casi che quel payload lo asseriscono. 🧯 Il primo sabotaggio (`PMO_WALLET_WRITE_ENABLED = false`) era **inerte** — l'harness scavalca quell'interruttore — ed è la stessa specie del «sabotaggio 7»: *un sabotaggio che non fa rosso non dice che la rete è debole, dice che hai rotto la cosa sbagliata.*
⚠️ **Residuo dichiarato**: l'impalcatura dei due banchi diverge ancora, e **in tutte e due le direzioni** — `main` ha un controllo su `svcAddMessage` che TEST non ha, TEST ha il tracciamento `WS_BLOCKED` che `main` non ha. Non è stato toccato: sono migliorie nate separatamente, e riconciliarle è un lavoro suo.

🚨 **Resta aperta per il RESTO, e la decisione è sua**: promuovere **`livello-dimostrato.ts`** al ponte del bot (oggi costa nulla e chiude la porta in anticipo), o chiudere dichiarando e farne voci a sé.
...
## 📦 CHIUSE — 13, 14 e 15/08/2026 — 24 voci
