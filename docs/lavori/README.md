...
### **128** — 🏷️ CASH · CARD · WALLET: le tre parole diventano quelle, OVUNQUE

🗣️ **Voce SUA**, 02/09/2026 notte, in due messaggi: prima la preoccupazione — *«metti che uno non
vuole pagare col borsellino perché si vuole tenere i soldi, ti chiedo la gentilezza di mettermi anche
cash e card»* — poi la decisione, in due colpi: ***«chiamiamolo ovunque wallet»*** e ***«e anche gli altri chiamiamoli
comunque cash e card, perché sono più brevi e mi aiutano quando apro la scheda della partita a non
andare a capo»***.

⭐ **La ragione che ha dato è una MISURA, non un gusto**: le parole corte non mandano a capo i
...
le finestre di conferma («Incassare 10,00 € in **Contanti**?»), la fascia, la colonna dell'elenco soci.
🩹 **Corretta il 03/09 facendo il lavoro: la sezione INCASSI era GIÀ a posto.** Questa riga la elencava
fra le cose da tradurre, e invece le tre pastiglie del riepilogo e le intestazioni della tabella
dicevano **già** «💵 Cash · 💳 Card · 👛 Wallet». ⇒ Un pezzo di lavoro dichiarato che non esisteva.
📌 *Una scheda scritta guardando il codice a colpo d'occhio elenca anche ciò che è già fatto: prima di
eseguirla si riguarda, o si «cura» una cosa sana.* ⇒ Oggi la stessa cosa ha **due nomi in due schermate**, che è il difetto
vero sotto la richiesta.

✅ **La preoccupazione era infondata, e l'ho misurata prima di rispondergli**: i tre bottoni si
disegnano **sempre tutti e tre**, e `cBtn`/`kBtn` si spengono **solo** quando l'incasso è disattivato
del tutto. L'unico condizionato al saldo è il Wallet (`wBtn.disabled = !okSaldo`). ⇒ Chi ha credito
può comunque pagare in contanti o con carta. Nessun lavoro da fare su questo: era una domanda, e la
risposta è una misura.

🔨 **Il lavoro è il RINOMINARE, ed è più grosso di come sembra.** Non è una parola, sono **tre**:
`Contanti → Cash`, `Carta → Card`, `Borsellino → Wallet`. Solo «borsellino» compare **109 volte** in
`index.html` (e 91 in `supabase/`, 40 nel banco, 30 nei documenti), e «contanti»/«carta» hanno lo
stesso problema aggravato: **«carta» è una parola comune** e compare anche dove non c'entra niente
col pagamento. Un `sed` cieco rompe due cose:

| dove | cos'è | se lo si rinomina |
|---|---|---|
| `view_members_borsellino` | **chiave di PERMESSO**, salvata nei profili staff sul database | la colonna, la fascia e il tab Pagamenti spariscono a chi li vedeva |
| `come: 'borsellino'`, `sel.borsellino`, `d.borsellino`, `pl.borsellino`, `_PAY_METHOD_LABEL`/`_ICON` | **chiavi di DATO** dell'aggregazione della sezione **Incassi** | i totali per metodo vanno a zero o spariscono dalla tabella |

📌 *In una stessa parola convivono l'etichetta e la chiave: la prima si traduce, la seconda no — e
si distinguono solo guardandole una per una.* ⇒ Si cambia il **testo visibile** (l'etichetta della
fascia, l'intestazione di colonna, «Saldo borsellino», i titoli «Ricarica/Storna», il valore
`'Borsellino'` dentro `_PAY_METHOD_LABEL`, i messaggi dell'assistente) e si lasciano **intatte** le
chiavi.

🔨 **FATTA il 03/09, in servizio su TEST 6.292 e PROD 6.279.** **51 sostituzioni una per una**, e
mai un `sed`. Toccate solo le **etichette**: pastiglie di chi ha già pagato, colonna «come» della scheda
socio, intestazione e ordinamento nell'elenco soci, fascia e statistica della scheda, finestre di
ricarica e storno, messaggi della chat staff, tooltip dei tre bottoni, etichetta del permesso, filtro
«Con wallet». ⛔ Intatte tutte e tre le famiglie di **chiavi**, e sono famiglie diverse fra loro: quella
di **permesso** (`view_members_borsellino`, che vive nei profili staff **sul database** — rinominarla
nel codice non rinomina i profili), quelle di **dato** dell'aggregazione Incassi, e i testi che
**Matchpoint scrive** e il worker clicca per testo.
🩹 **Curata anche la colonna «come» della scheda socio**, che scriveva la **chiave grezza in minuscolo**
(`contanti`/`carta`/`borsellino`): era **metà del difetto che lui aveva visto** — le due metà dello
stesso pannello che parlano due lingue.
🔪 **Guardia: `test/letichetta-si-traduce-la-chiave-no.test.mjs`**, e fa i **due mestieri opposti**,
provati con due sabotaggi: rimessa un'etichetta in italiano → rossa; rinominata la chiave di permesso →
rossa. È la forma che serve qui: una guardia che guardasse da un lato solo inviterebbe a sbagliare
dall'altro.
📏 **Verificato su TEST 6.292 con la console remota** che `_PAY_METHOD_LABEL` e `_pmoPagamentiBody`
stanno nello **stesso ambito** — vivono in due `<script>` diversi, quindi non era ovvio — e che
l'intestazione di colonna dice «Wallet».
⛔ **Cosa NON è stato visto, e va detto**: su TEST **nessun socio ha movimenti** (0 su 400 esaminati),
quindi la colonna «come» con dati veri non l'ha vista nessuno. Il meccanismo è provato, la resa no.
⏳ **RESTA APERTA** finché non si guarda una scheda socio con dei movimenti dentro.

⛔ **Era stata rimandata la sera del 02/09, per scelta dichiarata**: sono ~45 stringhe **per la sola «borsellino»**, e con le altre due si superano abbondantemente le cento; toccano anche le schermate
del **denaro**, e un rinomina fatto a metà lascia l'app che parla due lingue — peggio di entrambe.
Vuole un giro solo, con le schermate guardate dopo. ✅🗣️ **E la domanda che avevo lasciato aperta è CHIUSA: il bot NON si tocca.** Sue parole, 02/09
notte: *«non ti preoccupare che nel bot si chiama Borsellino. Il bot lo vedono i soci, il gestionale
di prod lo vede la segreteria. Devi solamente ricordare che sono la stessa cosa.»*
⇒ **Due parole per due pubblici**, ed è una scelta e non una svista: `Wallet` in segreteria (dove sta
accanto a `Cash` e `Card` e deve stare stretta in colonna), `Borsellino` al socio (dove nessuno ha
mai visto la parola inglese e non c'è nessuna colonna da rispettare). ⛔ La 128 vale **solo per il
gestionale**; il repo `assistente-padel-agent` resta com'è.

🚨⭐⭐ **E allora va scritta la TABELLA DEI NOMI, perché adesso la stessa cosa ne ha QUATTRO.** È il
prezzo della scelta qui sopra, e chi non ce l'ha davanti crede di guardare cose diverse:

| dove | come si chiama | si può cambiare? |
|---|---|---|
| gestionale (segreteria) | **Wallet** | ✅ è ciò che fa questa voce |
| bot (soci) | **Borsellino** | ⛔ scelta sua, resta |
| **Matchpoint**, dialog «forma di pagamento» | **«Saldo disponibile»** | 🚨 **MAI**: il worker clicca quella voce **per testo** (`MP_PAYMENT_SELECTORS.cobroMethodLabels.borsellino`). Cambiarla vuol dire che l'incasso dal wallet non trova più il bottone |
| codice e database | `wallet`, `borsellino`, `view_members_borsellino` | ⛔ sono chiavi, non etichette |
...
difetto — e «aggiusta».* Non scavalca niente.
...
## 📦 CHIUSE — dal 13/08 al 03/09/2026 — 107 voci
