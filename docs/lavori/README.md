...
- 🔓 **LA REGISTRAZIONE DI NUOVI UTENTI È APERTA, su PROD e su TEST.** `disable_signup` = **false**,
  provider **email attivo**, `mailer_autoconfirm` false (serve confermare, cioè una casella vera — nessun ostacolo).
  ⇒ **`authenticated` non è un cerchio chiuso: chiunque può entrarci.**
...
- 🚨 **E la conseguenza, misurata SUBITO ed ESEGUITA — `get_self_assessments_by_tokens` versa dati personali
  ...
  **rivolta al pubblico**. Su PROD sono **8** le `SECURITY DEFINER` concesse ad `authenticated` **senza guardia di
  ...
  ⛔ **La cura NON è una revoca**, ed è il motivo per cui questa nota si ferma qui: **l'app la chiama davvero**
  (`index.html:29298`, più una sonda di schema alla 28620) ⇒ revocarla ad `authenticated` **romperebbe il
  gestionale**. La strada giusta è una **guardia di staff dentro la funzione**, che è un cambio di comportamento su
  codice vivo. ⚖️ **Non promossa e non eseguita**: la decide il committente.
  📌 Stessa identica situazione su **TEST**.
