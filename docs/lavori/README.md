...
### **157** — 🔼 LA NOTA DI DUE RIGHE, E IL TITOLO SALE NELLA TESTATA — 🔨 **IN SERVIZIO su PROD 6.349**

🗣️ **Suo, il 04/09/2026 a notte**, guardando la scheda di una partita: *«Sulla parte sinistra c'è
il campo nota che è troppo alto, poi diminuirlo di parecchio, tanto di solito una nota che si mette
non è mai più di 2 righe. Mentre la parte sinistra la puoi alzare all'altezza del titolo della
scheda, fammi un mockup»* → tre letture disegnate, e lui: **«fai la A»**.

📏 **MISURATA LA SCHEDA VERA PRIMA DI DISEGNARE**, su PROD 6.346 a 1440×900: finestra 860, colonna
sinistra 330, titolo **41 px + 10 di margine**, e i pezzi della sinistra Slot 152 · Nota 161 ·
bottoni 87 · conto 120 = **521**, contro i **396** della colonna giocatori.

🎨 **Mockup** `mockup/scheda-nota-e-titolo-mockup.html`, e **si misura da sé**: la tabella sotto
ogni variante la calcola il browser aprendo la pagina.
🚨⭐ **E il mockup ha detto una cosa che non avevo previsto** — è il motivo per cui si disegna prima
di scrivere: l'altezza della finestra la detta **la più alta delle due colonne**. Con pochi
giocatori comanda la sinistra e accorciare la nota si vede tutto; con **quattro** le due quasi
pareggiano (484 contro 460) e da lì **ogni pixel tolto alla nota non lo vede più nessuno**. ⇒ La ①
da sola non basta, e la A vince perché il titolo è a **tutta larghezza**: toglierlo dalla griglia
guadagna **sempre**, chiunque delle due stia comandando.

🔨 **① La nota**: 3 righe → **2**, e il minimo da 64 a 48 ⇒ da **85 a 64 px**. ⚠️ Resta
**trascinabile** (`resize:vertical`): si toglie spazio sprecato, non una possibilità.
🔨 **② Il titolo** esce dal flusso e si posa nella **testata**, che conteneva solo la ✕ — spazio
già pagato, e vuoto. Le due colonne partono in cima.

⭐⭐ **SOLO CSS, il DOM non si tocca — e la misura che l'ha reso possibile**: nella catena
`.svc-edit-box` → `.svc-msg` → `#svcChatMessages` → `#svcChatPanel` **nessuno è
`position:relative`**, e il pannello è `fixed` ⇒ un figlio `absolute` risolve contro il **pannello**
e **non viene ritagliato** da `#svcChatMessages` benché quello sia `overflow:auto` (il ritaglio vale
solo per i discendenti il cui blocco contenitore sta dentro chi ritaglia).
⚖️ La strada ovvia era **spostare il nodo** nella testata: avrebbe funzionato, e avrebbe lasciato un
titolo che **sopravvive al box che l'ha creato** — una scheda chiusa e una testata che continua a
dire «Campo 3 · 14:00». Da `absolute` il titolo **vive e muore col box**, perché ci resta dentro.
📌 *Prima di spostare un nodo per ottenere una posizione, si guarda se la posizione si può
**chiedere**. Un nodo spostato porta con sé un ciclo di vita da sorvegliare.*
🚨 `pointer-events:none` non è un vezzo: la ✕ vive nella **stessa banda**, e un titolo che si prende
i click è la **voce 146 daccapo** — due bersagli vicini e la mira che trova quello sbagliato.

🩹⭐ **E UN DIFETTO CHE IL BANCO NON POTEVA VEDERE, trovato sulla pagina viva di TEST 6.348**: la
colonna dei giocatori restava a **280** mentre lo Slot era a **118**, cioè scivolata sotto invece di
partire in cima — proprio la cosa che la A doveva ottenere. 🔎 La causa vale più del difetto:
`grid-row:2 / span 10` esisteva perché la riga 1 era del **titolo**; tolto il titolo dal flusso quel
`2` descriveva **una vicina che non c'è più**, e nessuna prova poteva accorgersene perché la regola
era ancora lì, scritta bene, e sbagliata.
📌 *Un numero che descrive la posizione di qualcun altro invece della propria diventa falso quando
l'altro se ne va — e resta verde.*

🧪 **Banco** `test/il-titolo-sale-nella-testata.test.mjs`, **11 prove**, e le due che valgono
guardano le **premesse**: che nessun antenato diventi `position:relative` (il titolo tornerebbe
dentro il box e si accamperebbe sulla prima sezione, in silenzio) e che il titolo resti **figlio
diretto** del box. 📏 **Otto sabotaggi, otto rossi.**

✅⭐ **PROVATO SULLA PAGINA VIVA, su TEST 6.349 E su PROD 6.349, a TRE larghezze** (1280×800 ·
1440×900 · 1920×1080): titolo `absolute` alto **20 px** — una riga — **dentro la banda** della
testata, nota **64 px**, Slot e Giocatori **tutti e due a 118** ⇒ partono in cima, la **✕ risponde
ancora** al suo centro e il titolo **non si prende i click**.
...
## 📦 CHIUSE — dal 13/08 al 04/09/2026 — 122 voci
