...
| | |
|---|---|
| **117** | 🚫 **TOGLIERE UN GIOCATORE NON RIESCE: il click su «elimina» non va a termine** — 📏 **visto succedere due volte** il 01/09 su PROD: 22:22:08 (Laura, indice `_3`) e 22:27:23 (Marco, indice `_1`), stesso errore — `locator.click: Timeout 8000ms exceeded` sul link `…RepeaterParticipantes…LinkButtonEliminar_N`. Il link **viene trovato** (`locator resolved to <a…>`): non diventa cliccabile in tempo. 🚨 Il worker fa **un solo tentativo, 8 s, nessun retry**. ⚖️ Vale per il bot **e per l'app della segreteria**: stessa strada. ⛔ **Non è la sessione condivisa**: alle 22:29:45, dopo i due fallimenti, una lettura del tabellone sulla stessa sessione è andata a buon fine (`warm_fresh`, HTTP 200). 🔬⭐⭐ **CAUSA TROVATA E MECCANISMO LETTO PER INTERO, la notte del 01/09** — e la riga «un solo tentativo, 8 s, nessun retry» descriveva il **sintomo**: un timeout più lungo avrebbe fallito identico. 📏 Il call log di Playwright, letto con `stato-worker.yml`, dice chi intercetta: `<div id="mpModalDevolucionCapa"> … intercepts pointer events`. Il link ha `onclick="return confirmarEliminarParticipanteConDevolucion(this)"` ⇒ il **primo** click apre un modale di conferma **rimborso** e restituisce `false`, quindi il postback **non parte**; il worker non lo sa, aspetta 1,2 s, ri-scansiona, trova la persona ancora lì e **ri-clicca** — e il secondo cade sul modale aperto. 📌 I `steps` lo dicevano già senza il call log: `elimina:laura aprea` compare **due volte** nella stessa operazione. 🧾 **Il modale, letto dalla ficha viva** (sonda `leggi-ficha-matchpoint.yml`, sola lettura, zero click): `MPModalDevolucion.confirmar` chiede *«Cosa vuoi fare con i pagamenti?»* con **tre** opzioni — `devolverSaldoACobrados` («Rimborsare importi pagati»), `nada` («Niente»), `defecto` («Assegnato dal sistema», che è il `valorPorDefecto`) — poi scrive `CC_Datos_HiddenFieldConfirmaBorrarParticipante = 'True'` e `CC_Datos_HiddenFieldComportamientoDevolucionParticipante = <scelta>`, e **rilancia** l'elemento. ⇒ La cura non ha bisogno di guidare il modale: bastano i due campi nascosti più il `__doPostBack` che sta già nell'`href` del link, e il modale non si apre proprio. 🚨⭐⭐ **MA QUALE DELLE TRE OPZIONI È UNA DECISIONE SUA, NON DEL CODICE**: quel campo governa un **rimborso al socio**. `defecto` è il default di Matchpoint stesso — cioè quello che ottiene la segreteria che preme «Confermare» senza toccare niente — ed è la scelta che non inventa, ma va detta e non presa in silenzio. 🗓️ **E si sa da QUANDO**: tutti e quattro i link della ficha 9754 (01/09) hanno l'onclick, mentre il fallimento del **29/08** (ficha 9722, fabiola limuti) era su un link che l'onclick **non aveva** — lì l'intercettatore era un `fancybox` di `Compartir.aspx`, caso **diverso**. ⇒ Il modale è comparso **fra il 29/08 e il 01/09**: non è un difetto nostro maturato, è un **cambiamento di Matchpoint**. 🩹 Cade quindi la riga qui sotto — *«non si sa da quando»* — che era vera prima di questa misura. ✅⭐ **CURATA E IN SERVIZIO dalle 23:39 del 01/09** (worker deployato da `main`, health check verde): non si clicca più il link — si scrivono i due campi nascosti che il modale scriverebbe (`ConfirmaBorrarParticipante='True'`, `ComportamientoDevolucionParticipante='defecto'`) e si chiama il `__doPostBack` che sta già nell'`href`, così il modale **non si apre proprio**. Le fiche senza modale (lezioni, e le partite fino al 29/08) tengono la strada vecchia col `dismissSwalOk` in più. 🗣️ **`defecto` è una sua decisione**, non una scelta tecnica — quel campo governa il rimborso al socio — ed è anche il `valorPorDefecto` di Matchpoint ⇒ identico a quello che ottiene la segreteria premendo «Confermare» senza toccare niente. 🛡️ Con due protezioni che prima non c'erano e sono metà della cura: un **tetto ai giri** (il ciclo poteva ri-tentare la stessa persona senza limite — è il modo esatto in cui si arrivava al secondo click) e **l'invariante**, cioè che una rimozione riuscita fa sparire UNA riga, ri-guardata per ~6 s prima di dichiarare il contrario, perché leggere una volta sola trasformerebbe una scheda lenta in un «no» falso. ⛔ **RESTA APERTA, e il motivo è uno solo: nessuna rimozione vera è ancora passata da questo codice.** Il banco non copre `server.mjs` (automazione browser) e su TEST le scritture verso Matchpoint sono simulate ⇒ la prova fisica è **un togli vero su PROD**, e va chiesta a lui. 📏 Se fallisce, adesso il registro dice **perché**: `REMOVE_NON_APPLICATA`, `REMOVE_CONFIRM_FIELDS_MISSING`, `REMOVE_TROPPI_GIRI` — nomi, non un timeout muto. |
...
## 📦 CHIUSE — dal 13/08 al 01/09/2026 — 100 voci
