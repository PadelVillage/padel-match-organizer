...
- 🔑 **Il PIN admin: cosa resta dopo la 48, misurato e RIDIMENSIONATO.** Chiudendo la 48 avevo
  ...
  **eseguito** per provarlo, non dedotto (risposta `false` a un PIN sbagliato); ③ ma `authenticated`
  **non è il pubblico**: `auth.users` ha **6 utenti su PROD** e **5 su TEST**, cioè lo staff, non una
  popolazione aperta. ⇒ Il vettore non è «chiunque da Internet», è «chi ha già un account».
