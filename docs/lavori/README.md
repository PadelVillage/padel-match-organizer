...
| | |
|---|---|
| 🔴 **Urgenti** | **3** — la **65** e la **66**, curate e in servizio, aperte perché la cura non l'ha ancora vista succedere nessuno: **si aspettano, non si provocano**. 🆕 E la **74**, **promossa da lui il 23/08** dopo averla vista addosso a sé due volte — con la regola che l'accompagna: *«quando la segreteria fa un qualsiasi tipo di operazione, le persone che sono dentro la partita devono essere avvisate»*, *«corretti fino in fondo»*, *«vale anche per una lezione»*. Curata lo stesso giorno in due metà: cade il salto del primo dell'elenco (a proteggere la voce 70 resta la **ricevuta**, che sa *chi ha chiesto* invece di *chi è il primo*) e uno spostamento diventa un gesto suo. 📦 **Chiusa il 23/08 mattina: la 63**, a cura vista sul bersaglio — previsto 12:27:19 leggendo il codice, successo 12:27:20 |
| 📋 **In coda** | **7** — la **68** (curata e in servizio: resta finché il primo giorno vero non è stato guardato), la **69**, la **70** e la **71** entrate nella notte del 21/08, la **72** e la **73** del 22/08, più la **60** in sezione D (due passi su quattro vivi su TEST, in attesa che parli con Wansport). 🔼 **La 74 è uscita di qui il 23/08**: promossa fra le urgenti da lui, e chiusa la sua metà mancante lo stesso giorno |
| 📦 **Chiuse** | **61** il 13–23/08 + ~56 dal 7/08 + ~41 fino al 6/08 |
...
### **74** — 🔄 Uno SPOSTAMENTO manda solo la metà cattiva — PROMOSSA e CURATA il 23/08

🗣️⭐⭐ **PROMOSSA FRA LE URGENTI DAL COMMITTENTE il 23/08**, dopo averla vista addosso a sé due
volte di fila. E la sua promozione è arrivata insieme a **una regola**, che vale oltre questa voce:
...
📏 **IL DANNO, misurato sui suoi due spostamenti del 23/08** — non ricostruito: spostando la sua
partita dalle **09:30 campo 1** alle **11:30 campo 2** e poi indietro, in `pmo_eventi_staff` è nato
`annullata` per lo slot vecchio (visto_at 09:58:02 e 10:38:02) e **zero** `aggiunto` per quello
nuovo, **tutt'e due le volte**. Sul telefono: *«La tua partita non c'è più… è stata annullata dal
circolo»*, e nient'altro.
⇒ Non era solo un avviso mancante: quello che arrivava era **falso**. *Chi legge «annullata» non va
a cercare la partita altrove: la dà per persa.*
⭐ **La controprova stava nella stessa tabella**, su partite di altri: su `2026-08-24|15:00|2` sono
nati **tre** `aggiunto`, e chi mancava era **il primo dell'elenco**. ⇒ Su una partita di quattro si
perdeva un avviso su quattro; su una di **una persona sola** si perdeva l'unico.

🔨 **LA CURA, in due metà.**
**Ⓐ Il salto del primo dell'elenco cade, per tutti.** Era un **surrogato**: la domanda vera non è
«chi è il primo?» ma **«chi ha chiesto la scrittura?»**, e a quella risponde la **ricevuta** della
voce 70. Le due divergono esattamente dove il surrogato sbagliava — una partita scritta dalla
segreteria per un socio solo ha un primo che non ha chiesto niente.
⭐ **E la rete era già tesa, scritta apposta per questo giorno**: `consumer-booking-write` lascia una
ricevuta anche sulla `create` dal bot, col commento *«questa ricevuta oggi non copre niente, ed è una
RETE… regge il giorno in cui l'ordine cambiasse»*. 📏 Finestra 20′ (+3′ di tolleranza) contro un sync
che vede in ~2′ (massimo misurato 10′04″) ⇒ copre con margine, e `consumer-staff-events` **fallisce
chiuso**: senza ricevute non consegna niente.

**Ⓑ Uno spostamento diventa un gesto suo, `spostata`.** Il confronto guardava gli **slot**: uno
sparisce, un altro nasce, e da fuori è indistinguibile da un annullo più una prenotazione nuova.
⇒ Con l'identità della prenotazione le due cose si distinguono — **stessa prenotazione, slot diverso
= spostata** — e nasce **un** fatto invece di due scollegati.
📏 **La chiave è `numero`, non `idReserva`**, misurato su PROD: sulle **122** righe `booking` vive
`numero` c'è **122** volte e `idReserva` **70** (sta sulla capofila, manca sulle righe dei compagni),
e dove ci sono entrambi **non discordano mai**. È la stessa chiave che quella mattina aveva curato la
verifica dell'esito: là serviva a **contare**, qui a **seguire**.
⚖️ Uno spostamento che cambia anche i giocatori dice **tre cose diverse**: chi resta legge «spostata»,
chi è stato tolto «non sei più dentro» (con le coordinate **vecchie**, che del posto nuovo non deve
sapere niente), chi è stato messo «sei in campo».
⚠️ **Senza identità non si inventa niente**: si torna al comportamento di prima. Il verso in cui si
sbaglia resta quello di oggi — dire «annullata» di uno spostamento — non promettere una partita
altrove senza saperlo.

🚨 **L'ORDINE DI MESSA IN SERVIZIO, misurato e non supposto**: `ponte.ts` **scarta i gesti che non
conosce** (`return null`). ⇒ Deployando prima il gestionale, per la finestra fra i due deploy uno
spostamento non produrrebbe **nessun** avviso — peggio di oggi, dove almeno ne arriva uno sbagliato.
⇒ Prima il **bot** (inerte da solo: impara una parola che nessuno gli manda), poi la **migrazione**
(il `CHECK` ammetteva solo tre gesti), poi il **gestionale**.

📌 **La storia precedente di questa voce, dal 22/08**, resta qui sotto per intero — la prima metà
(le lezioni) è stata curata quel giorno, e il residuo che dichiarava *«uno spostamento manda ancora
due messaggi invece di uno che dice spostata alle 12:30»* è esattamente ciò che si chiude oggi:

🚨🔇 **Uno SPOSTAMENTO manda solo la metà cattiva: «la tua partita non c'è più», e poi silenzio** — 📏 **misurato il 22/08 pomeriggio**, su una domanda del committente fatta un'ora dopo aver visto funzionare la 73: *«ma invece a Maria Pia Bettiol è arrivata la prenotazione della lezione del venticinque agosto alle ore dodici e trenta?»*. **I fatti**: la lezione `idReserva 9500` di Maria Pia Bettiol si sposta dalle **10:00** alle **12:30** del 25/08, e il giro di sync delle 12:46:37 vede le due cose **insieme** — lo slot delle 10:00 sparisce, quello delle 12:30 nasce (la riga `booking|9500|2026-08-25|12:30|…` ha `created_at` **12:46:37**, lo stesso istante). In coda nasce **un solo fatto**: `annullata` per le 10:00, consegnato@12:47:35. Per le 12:30 **zero**. ⇒ Sul suo telefono è arrivato *«👋 La tua partita non c'è più… è stata annullata dal circolo»* e nient'altro: la lezione **esiste ancora**, spostata di due ore e mezza, e lei non lo sa. 🔎 **LA CAUSA, misurata e non dedotta — una regola giusta applicata dove non vale.** Le partite NUOVE producono `aggiunto` per tutti **tranne il primo dell'elenco**, che è l'organizzatore (`eventi-staff.ts`: *«il primo si salta perché è l'unico che la partita l'ha voluta»*). Ma in una lezione di un socio solo, il primo dell'elenco **è anche l'unico** ⇒ il ciclo salta l'unica persona da avvisare. 🚨 **E non riguarda solo gli spostamenti**: per costruzione **la segreteria che prenota a un socio SOLO non gliela annuncia MAI**, e il conteggio lo conferma — dei fatti `aggiunto` di sempre non ce n'è nessuno nato da uno slot con un nome solo. ⚖️ **È la stessa forma della 70 e della 71**: *un dato che risponde a due domande con lo stesso segno*. «Primo dell'elenco» vuol dire *«ha organizzato lui»* in una partita nata dal bot e *«è l'unico invitato»* in una lezione scritta dalla segreteria — e chi legge deve indovinare. 🗣️⭐⭐ **DECISO DAL COMMITTENTE lo stesso pomeriggio, e la sua frase è la regola**: *«quando si tratta di una lezione, quei giocatori che stanno nell'elenco devono ricevere la notifica»*. ✅ **CURATA (PR #977/#978 sul gestionale, #56 sul bot).** ⇒ Il salto del primo **resta sulle partite** — lì il primo è davvero chi l'ha voluta — e **cade sulle lezioni**, che un organizzatore fra i giocatori non ce l'hanno. ⚖️ E il salto era comunque un **surrogato**: la domanda vera non è «chi è il primo?», è «chi ha chiesto la scrittura?», e a quella risponde la **ricevuta** della voce 70, che copre tutte e cinque le scritture del ponte, `create` compresa ⇒ togliere il salto nelle lezioni **non riapre** la 70. 📏 Misurato prima di scrivere: `tipo` ha due soli valori su PROD — `Partita` (3030) e `Lezione Libera` (1230) — quindi la distinzione era già nel dato. 🚨 **E la lezione si riconosce dalla parola «lezione», non da «diverso da Partita»**: col secondo criterio un tipo nuovo (un torneo, una manutenzione) diventerebbe una lezione **in silenzio**, e il sabotaggio lo mostra — scritto così fa cadere anche due prove vecchie della voce 70. 🔒 **La seconda metà, senza la quale la prima fa danno**: tutte le frasi del bot dicevano «partita», e a Maria Pia era arrivato *«la tua **partita** non c'è più»* per una lezione. Ora la parola la sceglie il **gestionale** e il bot la usa — e al bot arriva `lezione`/`partita`, **non** `Lezione Libera`, che è una parola di Matchpoint e per la regola ferrea del 19/08 al bot non deve arrivare: *il giorno del distacco quella colonna e il bot non si toccano*. ⚠️ Assente o sconosciuto ⇒ «partita», il comportamento di prima. 🔨 8 casi nuovi di qua (40 in `eventi-staff.test.ts`) e 4 di là, più due prove sulla riduzione; sabotaggi 5 su 5. 📌 **Resta fuori, dichiarato**: uno spostamento manda ancora **due** messaggi («non c'è più» + «sei in campo») invece di uno che dice *«spostata alle 12:30»*. È un secondo pezzo e va deciso a parte. ✅⭐⭐ **VISTA FUNZIONARE la sera stessa, su una lezione vera prenotata dal committente** (lunedì 24/08, 09:00, campo 1, lui unico allievo — cioè esattamente il caso di Maria Pia). **La catena al secondo**: 18:53:23 salva dal gestionale · 18:54:03 il sync guarda · **18:54:52 nasce il fatto** (`aggiunto` · `tipo: lezione`) · **consegnato@18:56:12**, e il telefono segna **18:56**. ⇒ Gesto → messaggio **2′49″**, e prima di oggi quel fatto **non sarebbe nato affatto**. 🗣️ **E il messaggio vero ha prodotto due correzioni che rileggendo il codice non si vedevano** (PR #57): ① il **titolo** era l'unico dei tre a non dire di che si tratta — «Sei in campo» — ed è la riga che si legge per prima, spesso l'unica visibile nell'anteprima della notifica ⇒ ora «Hai una lezione»; ② *«l'italiano è sbagliato, "ti ha messo in lezione il circolo" no»* — e aveva ragione: *mettere in partita* è un modo di dire, *mettere in lezione* no ⇒ «Ti ha **iscritto alla** lezione il circolo». 📌 **La lezione di metodo è la ②**: avevo tradotto **scambiando un sostantivo dentro una frase scritta per l'altro** — un errore che il banco vede **verde** e un madrelingua vede subito. Ogni parola vuole il verbo suo. La prova ora vieta la forma vecchia per nome. 🔄 **E dalla stessa schermata è nato un terzo cambio** (PR #58): l'elenco si chiamava «Le mie partite» e mostra anche le lezioni **da sempre** ⇒ ora «**Le mie prenotazioni**» (e «Non hai prenotazioni» quando è vuoto). *Un nome che descrive metà del contenuto regge finché l'altra metà è rara.* 🚨 **E una mia affermazione SBAGLIATA, corretta dalla sua schermata**: avevo scritto che «il bot mostra il bottone annulla anche sulle lezioni, la porta è aperta adesso». **Falso** — le lezioni sono fuori dai poteri del bot da prima (`eLezione` in `avvisi.ts`, usata da `lascia-partita.ts` e `invito-partita.ts`), e la scheda dice già *«per modificare o gestire una lezione, chiama la segreteria»*. ⇒ La sua proposta *«annullare una lezione lo può fare solo la segreteria»* **è già la regola in servizio**. ⚖️ Come ci sono arrivato è la parte da tenere: ho cercato i consumatori di `.tipo` con un `grep … | head -10`, l'elenco era più lungo di dieci righe, e **ho concluso un'assenza da una lista troncata**. È la trappola che questo progetto ha già pagato due volte — *un'assenza è una misura solo se la finestra la contiene* — e stavolta l'ho fatta con la finestra decisa da me.
