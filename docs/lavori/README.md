...
🆕 **2️⃣ APRIRE UNA PARTITA SU PROD E GUARDARE LA TESTA DELLA SCHEDA** *(voce 127, chiesto il 02/09
notte)*. La cura è in servizio su TEST 6.290 e il banco è verde, ma **nessuno l'ha ancora vista**:
✋ un task non è finito finché non lo si è provato fisicamente. Basta aprire una partita e guardare
se «Modifica prenotazione» c'è.

🆕 **3️⃣ TENERE APERTO IL CALENDARIO SU PROD QUALCHE MINUTO** *(voce 130, chiesto il 02/09 notte)*.
Il saltello di 24px che ha segnalato lui è misurato e curato; resta da vedere che non ci sia più.
...
### **127** — 🖼️ LA SCHEDA DELLA PARTITA SI APRE TAGLIATA IN ALTO

🗣️ **Voce SUA**, 02/09/2026 notte: *«quando clicco dentro una partita si apre la scheda della partita
in maniera sbagliata. Non vedo la parte alta.»* 📸 Con schermata: nel pannello di sinistra la riga
**«Modifica prenotazione»** è tagliata dal bordo superiore — si legge mezza, e sotto comincia
subito «Campo 2 · 02/09/2026 · 18:00».

📏 **Presente anche nella sua schermata precedente della stessa sera, su PROD 6.273** ⇒ **non è nato
oggi** e non lo hanno introdotto né la 125 né la 126: quelle non toccano il pannello della partita.
⚖️ È una constatazione, non una diagnosi: dice *da quando NON è*, non *da quando è*.

⛔ **NON MISURATO, e va detto perché la strada ovvia è chiusa.** Ho provato a riprodurlo con
`tools/verifica-browser` su PROD e **il calendario non si popola**: la console blocca di proposito
tutto `/functions/v1/`, che è la strada da cui il calendario staff prende le prenotazioni ⇒ zero
riquadri cliccabili, zero pannelli da aprire. La sonda ha risposto `nessuna prenotazione cliccabile`,
e quella risposta parla dell'**attrezzo**, non dell'app.
📌 *Una sonda che non raggiunge il caso non dice «non succede»: dice «non ho guardato».*

⇒ **Serve il suo schermo, o il Mac.** Le due domande da fare lì, e sono cinque minuti: ① il pannello
è **scrollato** (il titolo esiste ma sta sopra il bordo) o **tagliato** (il contenitore parte più in
alto della finestra)? ② succede **sempre** o solo quando la lista giocatori sta ancora caricando —
nelle sue due schermate il pannello mostrava *«Aggiorno la lista giocatori da Matchpoint…»* in
tutt'e due i casi, e un contenuto che cresce dopo l'apertura è il sospettato numero uno, per la
stessa ragione della cella «Bot Telegram» della 118.
🔪 Chi ci mette mano scriva la guardia **sul fatto** (il titolo dev'essere dentro il rettangolo
visibile del pannello), non sulla misura di un CSS. Non scavalca niente.

---

🔄🩹 **MISURATA E CURATA il 02/09 notte — e le due domande che stavano qui NON sono servite.**
Qui sopra c'era scritto *«serve il suo schermo, o il Mac»*: era vero della strada che avevo provato
(cliccare una prenotazione nel calendario), **falso del difetto**. La scheda si apre anche
**chiamandola per nome** — `staffCalEditPlayers(iso, campo, ora, …)` non ha bisogno di nessun
riquadro da cliccare, perché costruisce il pannello dai suoi argomenti.
📌 *Una sonda che non raggiunge il caso dice «non ho guardato» — ma prima di dichiarare che serve
un altro paio di occhi conviene chiedersi se la porta di servizio è aperta.*

📏 **La misura, su PROD 6.276**, spazzando l'altezza utile del riquadro con la stessa scheda dentro:

| altezza utile | eccedenza del pannello | titolo |
|---|---|---|
| 1000px | 281px | ✅ si vede |
| **1050px** | **231px** | ❌ tagliato di **221px** |
| 1100px | 181px | ❌ tagliato di 171px |
| 1200px | 81px | ❌ tagliato di 71px |
| 1300px | 0 | ✅ si vede |

⇒ **È SCROLLATO, non tagliato** — la prima delle due domande ha risposta, e la risposta cambia dove
si cura. La chat dello staff ha un auto-scroll con la regola *«se manca meno di **260px** alla fine,
incollati in fondo»*: una scheda che sborda di **poco** ci cade dentro tutta intera e si porta via
la testa. Sborda tanto (finestra piccola) → non scatta; ci sta tutta → non c'è niente da scorrere.
🚨 **È il motivo per cui non capitava sempre e rileggendo il CSS non si vedeva niente**: il difetto
sta in una soglia, non in una regola di stile — e colpisce **proprio sullo schermo normale**, cioè
dove la scheda sborda di poco.
🚨 E si ripeteva a raffica: la nota *«⏳ Aggiorno la lista giocatori… (Ns)»* **riscrive sé stessa
ogni secondo** ⇒ l'observer ripartiva ogni secondo, per un minuto. Anche la seconda domanda ha
risposta: il legame col caricamento c'era, ma non era il contenuto che cresce — era il **contatore
dei secondi** a rifare scattare la regola.

🔨 **La cura (v6.290): un TETTO all'auto-scroll.** Non si toglie l'auto-scroll — *«il messaggio nuovo
va IN FONDO»* è una decisione presa, e in chat è giusta. Si scorre in fondo come sempre, **ma mai
oltre la cima della scheda** (`_svcTettoScroll` / `_svcAutoScroll`). E l'observer non strappa **mai**
verso l'alto chi sta leggendo (`soloInGiu`), o il contatore dei secondi lo riporterebbe su ogni volta.
📌 *Una regola che vale per i messaggi non vale per un modulo che sta nello stesso contenitore: il
contenitore è lo stesso, il gesto di lettura no.*
⚖️ **Il difetto non stava in UN punto: `scrollTop = scrollHeight` compariva in CINQUE**, e ne bastava
uno vivo per rimettere il titolo sopra il bordo — compreso `staffCalBringEditorToTop`, che sposta il
box **fuori dalla sua bolla** (motivo per cui la guardia si aggancia a `.svc-edit-box` e non a
`.svc-edit-host`: agganciata all'host avrebbe perso la scheda proprio nel percorso dell'assistente).

🔪 **Guardia: `test/la-scheda-si-legge-dallalto.test.mjs`** — 8 casi, scritta **sul fatto** come
chiedeva questa scheda: *la cima della scheda dev'essere dentro il rettangolo visibile*, spazzando
tutte le altezze da 300 a 1400px. Non misura nessun CSS. In più chiude la **classe**: nessuno può
più scrivere `x.scrollTop = x.scrollHeight` scavalcando il tetto.

⏳ **RESTA APERTA**: ✋ un task non è finito finché non lo si è provato **fisicamente**. Il banco è
verde e la misura è di PROD, ma nessuno ha ancora **aperto** una scheda partita e visto il titolo al
suo posto. ⇒ Si chiude quando lui apre una partita su PROD e la testa c'è.
...
### **130** — 🗓️ IL CALENDARIO CHE «SALE E SCENDE»: non era il calendario

🗣️ **Osservazione SUA**, 02/09/2026 notte, mentre stavo già guardando la 127: *«guarda anche perché
il calendario ogni tanto sembra che si sposta. Quindi sale e scende velocemente. Ma è molto piccolo
lo spostamento in verticale.»*

📏 **MISURATA su TEST con la console remota**, e la misura è arrivata prima di qualunque ipotesi
perché ho usato lo strumento che il browser ha apposta — `PerformanceObserver` su `layout-shift`,
che non dice solo *quanto*, dice **chi**:
...
⇒ Due spostamenti di **24px**, a **4 secondi** l'uno dall'altro, ed è esattamente il «sale e scende,
molto piccolo». Poi, forzando visibile l'etichetta: intestazione **56 → 80px**, `#staffCalV36`
**189 → 213px** — le stesse cifre, quindi non è una coincidenza.

🚨 **La causa: `#staffCalCloudStatus`, l'etichetta grigia «aggiorno…»**, che si accende e si spegne
con `display` **dentro il flusso** dell'intestazione del calendario. Non è il calendario a muoversi:
è l'intestazione a **crescere sopra di lui**, e a spingerlo giù di una riga da 24px.
⚖️ Il poll ravvicinato (~8s) **passava già** `silent:true` apposta per non farla lampeggiare: quello
che restava sono i refresh «veri» (push realtime da un altro dispositivo, rientro sulla scheda,
...
📌 *Un avviso transitorio non ha diritto allo spazio di uno permanente.*

🔪 **Guardia: `test/letichetta-non-sposta-il-calendario.test.mjs`** — 3 casi, e protegge **tutt'e due
i versi**: che l'etichetta transitoria resti fuori dal flusso, **e** che quella permanente non ci
finisca per «coerenza». Una cura applicata due volte per simmetria è il modo tipico in cui questa
riparazione si disfa.

⏳ **RESTA APERTA**: la misura dice che l'etichetta è dichiarata fuori dal flusso, non che sullo
schermo non si muova più niente. ⇒ Si chiude quando lui tiene aperto il calendario su PROD qualche
minuto e non lo vede più saltellare.
🩹 Nasce da una **sua parola**, non da un'idea di fine giornata: la delega copre l'ordine delle voci,
...
## 📦 CHIUSE — dal 13/08 al 02/09/2026 — 104 voci
