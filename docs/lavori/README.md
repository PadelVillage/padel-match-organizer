...
| | |
|---|---|
| **116** | 🗣️ **UN GESTO DEL SOCIO ARRIVA AGLI ALTRI FIRMATO «IL CIRCOLO»** — 📏 trovato dalla **prova fisica della 88**, non rileggendo: 01/09 ore 20:01, Laura Aprea entra da sé in una partita aperta e a Maurizio, Marco e Lidia arriva *«Entra Laura Aprea. **L'ha cambiata il circolo.** Se non te lo aspettavi, **chiedi in segreteria**»* — screenshot di tutti e tre. ⚖️ **Due bugie**: l'**attribuzione** (il circolo non ha fatto niente) e la **strada** (la segreteria di quel gesto non sa nulla e non può rispondere). 🚨 **Il difetto è della 79, e la 88 l'ha solo fatto vedere**: la frase era fissa perché ogni fatto nasceva dal sync, cioè dalla segreteria — dal 31/08 lo stesso tubo porta anche i gesti dei soci. ⭐⭐ E **non bastava dedurlo da `origine`**: `'conferma'` vuol dire «registrato all'istante della conferma» e ci passa **anche la segreteria dall'app** — 📏 delle **16** righe `annullata` così, chi le abbia chieste **non lo sapeva nemmeno il gestionale**. Un dato mai scritto non si recupera guardandone un altro. 🔨 **CURATA E IN SERVIZIO la sera stessa**, nell'ordine obbligato: bot ([#116](https://github.com/PadelVillage/assistente-padel-agent/pull/116), VM 20:23:54 — senza il campo è **inerte**), migrazione `chiesto_da` su `qqbf` e `cudi`, poi le edge ([#1240](https://github.com/PadelVillage/padel-match-organizer/pull/1240), 20:44). Curati **tutti e cinque** i gesti, non solo quello visto sbagliare. ⛔ **APERTA per la PROVA FISICA**: nessuno ha ancora letto *«L'ha chiesto ⟨nome⟩»* su un telefono. |
...
## 📦 CHIUSE — dal 13/08 al 01/09/2026 — 99 voci
