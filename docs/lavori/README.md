...
voce archiviata, e sta qui e non là per una ragione precisa: **un residuo lasciato dentro una voce
chiusa è un residuo perso** — è la stessa forma della 124, aperta da lui chiudendo la 84.

🗣️ L'idea originale, 02/09 notte: *«potrebbe essere carino poter cliccare sul nome del socio e gli
apre la scheda di anagrafica. Ma non so se si può fare.»*

✅ **Si può, ed è misurato**: `switchTab` e `openMemberCard(id)` esistono già, e il socio si trova
confrontando `p.idCliente` del roster con `pmoIdMatchpoint(g)` dei `giocatori`. Il pezzo tecnico
non è il problema.

🚨 **IL PREZZO, che va deciso PRIMA di scriverla e non dopo**: aprire la scheda socio vuol dire
**lasciare il calendario**, e il pannello della partita può avere **modifiche non salvate** in
...
· **ⓑ** si **chiede conferma** («hai modifiche non salvate, esco lo stesso?») — sempre cliccabile,
  al prezzo di un popup nel mezzo;
· **ⓒ** la scheda socio si apre **sopra**, senza cambiare tab — niente da perdere e niente da
  chiedere, ma è la più lunga da fare.
...
## 📦 CHIUSE — dal 13/08 al 05/09/2026 — 135 voci
