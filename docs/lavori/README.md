...
🗣️ **Suo, il 04/09/2026 a notte**, mentre andava a collaudare la 154 e la 155: *«nel menù in alto
c'è una parola che si chiama amministrazione che può essere confusa con l'amministrazione
economica. cambiamo il nome cosa suggerisci?»* → proposto **Impostazioni**, e lui: *«va bene
impostazioni»*.

📏 **GUARDATO COSA C'È DENTRO prima di proporre il nome**, perché un'etichetta deve descrivere
quello che apre: **Utenti Staff · Notifiche staff · Dati · Bot Telegram** (su TEST anche
**Circoli**). Non una riga di soldi — gli incassi hanno un capitolo tutto loro.
⇒ La confusione che ha visto **non era un'impressione**: il nome prometteva una cosa e il menu ne
apriva un'altra. Le altre due candidate sono state scartate per una ragione, non per gusto:
*Configurazione* è fredda e lunga per una barra che sta su una riga; *Sistema* suona tecnica, e
«Utenti Staff» non è sistema, è organizzazione.

🔨 **CAMBIATE TUTTE LE 24 OCCORRENZE CHE SI LEGGONO, non solo il menu**: il titolo della sezione,
la voce del cassetto sul telefono, l'etichetta del capitolo, il permesso nella scheda utente, e —
soprattutto — i **quattro testi del LOGIN**. 📌 *È lì che va a cercare chi non riesce a entrare:
lasciarne uno indietro manda una persona reale a cercare una voce che non esiste più.*
🩹 Tre punti riscritti a mano invece che sostituiti: il nome nuovo è di **genere diverso**
(«un'Amministrazione vuota» sarebbe diventato «un'Impostazioni vuota»).

🚨🚨 **LA CHIAVE INTERNA `administration` NON SI TOCCA**, ed è il vero pericolo di questa voce: ci
sono appese la visibilità per ruolo (`view_administration`, `data-section-key`) e i salti
`goToTabSection('administration', …)`. Chi «completa» la rinomina cambiando anche quella **non
rompe una scritta: spegne un capitolo intero per tutti**, e lo fa in silenzio, perché il menu
continua a disegnarsi. Due prove del banco stanno lì.

⚖️⭐ **E IL CONFLITTO DELLA PROMOZIONE HA MOSTRATO UNA COSA VERA**: su `main` la voce **«Circoli»
non esiste** — è lavoro che vive su `test-preview` e non è ancora promosso. Risolto tenendo la
frase di PROD e cambiando **solo** il nome. 📌 *Una promozione porta le righe del suo fix, non
quelle che le stanno accanto nel diff.*
🩹 Per la stessa ragione il banco elenca le **quattro** voci comuni ai due rami e non cinque: un
banco che pretende «Circoli» sarebbe **rosso su PROD** per un fatto che non riguarda questa cura.
📌 *Un banco che viaggia con una promozione deve reggere sui DUE rami, o non è una garanzia: è un
pedaggio.*

🧪 **Banco** `test/il-menu-non-dice-amministrazione.test.mjs`, **7 prove**. 📏 **Tre sabotaggi, tre
rossi**: un solo messaggio del login lasciato indietro, la chiave rinominata, un salto tradotto col
nome invece della chiave.

✅⭐ **PROVATO SULLA PAGINA VIVA, su TEST 6.346 e su PROD 6.346**: menu **«Impostazioni ▾»**, i figli
intatti, la navigazione arriva alla sezione (titolo `<h2>` ed etichetta `🔐 Impostazioni`), e la
parola vecchia **non compare più in nessun punto della pagina**.
✅ **E la pagina di ACCESSO, guardata da sola** (senza login, che è l'unico modo di vederla): *«Accedi
con l'email autorizzata in Impostazioni…»* e *«…solo per email autorizzate in Impostazioni.»*

🩹 **RESIDUO DICHIARATO, non curato**: la parola resta in **3 commenti** di due edge
(`staff-create-access`, `assessment-email-send`) e in un nome di prova del bot. Non sono testi che
qualcuno legge nell'app, e toccare `supabase/functions/**` **rideploierebbe due funzioni vive** per
un fatto cosmetico. Si cureranno alla prima modifica vera di quelle funzioni.

### **155** — 📌 LA BARRA DEI CAPITOLI RESTA FERMA, IL SOTTO SCORRE — 🔨 **CURATA e PROVATA**

🗣️ **Suo, il 04/09/2026 a notte**, con la schermata della barra: *«questa barra su desktop deve
rimanere sempre fissa e il sotto scollare»*.

📏 **MISURATO SULLA PAGINA VIVA PRIMA DI SCRIVERE UNA RIGA**, ed è il pezzo che conta: `sticky`
non funziona «in generale», funziona rispetto a **un antenato preciso** — e qui gli antenati non
sono quelli che sembrano.
· a scorrere **NON è il documento**: `html` e `body` sono `overflow:hidden`, e
  `scrollHeight − clientHeight` valeva **ZERO**;
· lo scroller è **`main.main-content`** (`overflow-y:auto`, 100vh, 213 px da scorrere);
· fra la barra e lo scroller c'è **un solo** antenato, `.container`, ed è **`overflow:visible`** —
  che è esattamente la condizione perché lo sticky viva.
📌 *Nessuna di queste tre cose si vedeva leggendo il CSS: `overflow:hidden` su `body` è dichiarato
seicento righe più in là, in un altro blocco.*

🔨 `position:sticky; top:0; z-index:1200`, dentro `@media (min-width:900px)`.
⚖️ **`top:0` e non `top:20px`**: i 20 px sopra sono il **padding di `.container`**, e fermandosi lì
il contenuto scorrerebbe *dentro* quella fascia, sopra la barra.
🚨 **1200 sta SOTTO il velo delle due schede** (`.svc-chat-overlay`, 2550): con una scheda aperta la
barra dev'essere coperta come tutto il resto. *Una barra accesa sopra un velo scuro non sembra
fissa, sembra rotta.*

🧪 **Banco** `test/la-barra-dei-capitoli-resta-ferma.test.mjs`, **6 prove** — e le due che valgono
non guardano la regola, guardano le **PREMESSE**: che `.container` non dichiari `overflow` (se
glielo mettono, la barra si aggancia a quel riquadro e smette di seguire lo scorrimento **senza
rompere niente a voce alta**) e che `.main-content` resti lo scroller.
📏 **Cinque sabotaggi, cinque rossi.**
🩹 E la prima versione della prova ② era **rossa su una cura giusta**: cercava la `@media` con
`lastIndexOf`, cioè «la più vicina indietro», che fra l'apertura e la regola trova quelle **già
chiuse**. Ora conta con una pila. 📌 *Una sonda che cerca «il più vicino indietro» in un testo
annidato non trova il contenitore: trova il vicino.*

✅⭐ **PROVA FISICA su TEST 6.344, a DUE larghezze** (1280×800 e 1440×900), e con il **prima**
misurato sulla stessa pagina:
· **prima**: scorrendo, la barra se ne andava a **−40 · −130 · −193**;
· **dopo**: resta a **`top:0`** a ogni altezza di scorrimento;
...
## 📦 CHIUSE — dal 13/08 al 04/09/2026 — 123 voci
