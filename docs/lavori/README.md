...
### **134** — 👁️ UN RISULTATO NON DEVE RESTARE NASCOSTO (la conferma sotto il bordo)

🗣️ **Nasce da una SUA DOMANDA**, 03/09/2026, subito dopo aver provato la 132: *«ogni volta che
faccio un'operazione che poi aspetto la conferma da Matchpoint, dove secondo te è giusto che io
veda la conferma? In alto alla scheda della partita oppure in basso?»* — poi precisata, ed è la
frase che definisce la voce: *«è solo una questione di visualizzazione di un risultato che non deve
essere nascosto, sia se scrivi sia se clicchi i bottoni»*.

📏 **MISURATO su PROD 6.281 con la console remota** (1440×900, scheda di Campo 2 · 03/09 · 16:30):

| momento | dov'è il messaggio nuovo |
|---|---|
| pannello 711px, scheda alta **1216px** (sborda di 505) | — |
| fermo sul bottone **Salva** (barra visibile) | ✅ visibile, −47px dal bordo |
| **subito dopo il click** | ❌ **568px sotto il bordo**, e `scrollTop` **torna a 0** |

🚨⭐⭐ **E il secondo difetto non è quello che sembra: non era il messaggio ad andare in basso, era
la VISTA a tornare in cima.** `svcAddMessage` chiamava `_svcAutoScroll(container)` **senza**
`soloInGiu`, quindi il tetto della **127** — nato per dire *«non oltre la cima della scheda»* —
veniva applicato anche a chi stava **più in basso**, e lo trascinava **indietro**.
📌 *Un tetto dice «non oltre», non «torna indietro».*
⚖️ L'observer quella regola ce l'aveva **già** (`soloInGiu`, ed è scritto nel suo banco); questa
seconda strada no. 📌 *Due strade per lo stesso gesto, e una sola protetta: la guardia esisteva e
guardava dalla parte giusta — semplicemente non copriva l'altra porta.*

🔨 **LA CURA, in tre pezzi — e nessuno alza il tetto della 127**, che resta giusto (una scheda si
legge dall'alto):
① `svcAddMessage` non strappa più verso l'**alto**;
② l'esito dei bottoni si scrive **DENTRO la scheda**, sopra la barra dei bottoni — perché è lì che
   sta l'occhio nell'istante del click, e i bottoni (Salva · Cash · Card · Wallet · ↩︎) stanno lì.
   ⛔ Non sostituisce il messaggio in chat, lo **affianca**: la chat è il registro di ciò che è
   successo e sopravvive alla scheda; questa riga è l'eco immediata e muore con lei;
③ una **pastiglia** «↓ …» incollata al bordo basso, che porta con sé il **testo** dell'ultimo
   messaggio ⇒ l'esito si **legge senza cliccare**, e si raggiunge con un dito. Sparisce da sé
   appena il messaggio entra nel bordo, anche scorrendo a mano.

⭐ **Il colore della riga d'esito si DEDUCE dal segno che la frase porta già in testa** (✅ · ⚠️ ❌ ·
...
🩹⭐ **E LA GUARDIA DELLA 127 È DIVENTATA ROSSA, GIUSTAMENTE.** Vietava `scrollTop = scrollHeight`
in tutto il file; la pastiglia, **cliccata**, deve andare davvero in fondo (passando da
`_svcAutoScroll` si fermerebbe alla cima della scheda, cioè non farebbe niente).
⇒ La guardia è stata **riportata sul fatto**, non allentata: il fatto che difende non è «nessuno
arriva mai in fondo», è **«nessuno ci va senza che l'operatore l'abbia chiesto»**. L'unico scavalco
ammesso sta dentro il gestore del `click` della pastiglia, e il banco pretende che resti **uno
solo**. 📌 *Una guardia che si rompe su un caso legittimo va resa più PRECISA, non più permissiva.*

🔪 **Banco nuovo**: `test/il-risultato-non-si-nasconde.test.mjs`, 11 casi. Il caso sul colore
**esegue** la regola ritagliata dal sorgente invece di rileggerla — e ha già trovato un difetto
**nel banco stesso** (il ritaglio perdeva `tipo ||`, quindi il caso «il tipo dichiarato vince»
...
aperta, che è l'altra metà della sua richiesta.
...
## 📦 CHIUSE — dal 13/08 al 03/09/2026 — 110 voci
