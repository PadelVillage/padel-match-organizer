...
tutte (180). Tutte discendono da una sua frase — **«il gestionale di TEST diventa il vero»** — che
...
| **183** | 📐 **LA GRIGLIA NON VIENE FATTA RISPETTARE, e i prezzi non ci sono** — 📏 misurato: `create` controlla il campo, la finestra 07:00-23:30, la durata 30-180 e i 30 giorni di anticipo — **mai** che l'ora sia **sulla griglia**. Ora che le fasce sono una tabella (voce 176), si può. ⛔ **E qui la coda è dovuta a LUI, non a noi**: 📏 il prezzo è `null` su **tutte e 41** le fasce — *«non ancora deciso»*, che è **diverso da gratis**, e il vincolo del database tiene le due cose separate apposta. Ha scelto **prezzo per GIOCATORE** (non il campo da dividere), ma **i numeri non li ha ancora dati**. ⚠️ **E non esiste NIENTE di quello che sembra esistere**: nessun limite al numero di partite aperte, nessun preavviso di disdetta (lui pensa siano definiti — *«abbiamo definito le regole nel fare il chatbot»* — ma nel codice ci sono solo quei quattro controlli), **nessuna tabella dei campi** (sono 4, cablati in **tre punti diversi**), **nessun listino** (le uniche cifre, 40/32, sono una stima da cruscotto **per partita**), **nessun modello di chiusure/festività**. 📌 *Una regola che tutti credono scritta è più pericolosa di una che manca: nessuno la va a cercare.* |
...
## 📦 CHIUSE — dal 13/08 al 08/09/2026 — 170 voci
