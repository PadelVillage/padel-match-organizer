...
⛔ **NON fatto stasera, per scelta dichiarata**: sono ~45 stringhe **per la sola «borsellino»**, e con le altre due si superano abbondantemente le cento; toccano anche le schermate
...
### **131** — 🆕 LA SCHEDA «NUOVA PRENOTAZIONE»: testa tagliata, e i campi su una riga

🗣️ **Voce SUA**, 03/09/2026 mattina, due cose in un messaggio: *«prima aggiusta la scheda quando apro
una nuova prenotazione che è tagliata in alto. Sempre sulla stessa, ottimizza i campi mettendoli
tutti su una riga e la data la metti dopo il testo campo. Fammi vedere un esempio prima di
sviluppare.»*

⭐ **«Fammi vedere un esempio prima di sviluppare» è la parte che ha cambiato il lavoro**, e va
scritta: sono state disegnate **tre** disposizioni alla larghezza vera, misurate, e lui ha scelto la
**A**. Senza quel passaggio avrei scritto la **B** — la più corta, quella che *sembra* la lettura
letterale di «tutti su una riga» — e avrei perso i quattro pallini colorati.
...
#### ① La testa tagliata — la 127 su un'altra scheda

📏 **Riprodotta su PROD 6.277**, e la prima sonda aveva sbagliato la sequenza: aperta la schedina
**senza** che prima passasse un messaggio dalla chat, non si taglia — perché è `svcAddMessage` a
installare l'auto-scroll sul contenitore. ⇒ Serviva la sequenza **vera**.
📌 *Una sonda che salta un passo dell'antefatto non misura il caso: misura un caso vicino, e risponde
«non succede» con la stessa sicurezza.*

| contenitore | eccedenza | testa |
|---|---|---|
| 400px | 281px | ✅ |
| **500px** | **181px** | ❌ tagliata di 171px |
| 600px | 81px | ❌ tagliata di 71px |
| 700px | 0 | ✅ |

⇒ **Identica alla 127**, soglia dei 260px compresa. 🚨⭐⭐ **Ma la cura della 127 non la copriva**: il
tetto riconosceva la scheda da `.svc-edit-box`, e questa è `.svc-flow-card`.
⚖️ *La cura era giusta, l'ELENCO era corto.* ⇒ Ora l'elenco è una costante dichiarata
(`SVC_SCHEDE_CHE_SI_LEGGONO_DALLALTO`) e — la parte che conta — **il banco conta da sé le schede che
il sorgente crea** invece di fidarsi di una lista scritta a mano: una lista a mano dentro una
guardia ha esattamente il difetto della lista che la guardia sorveglia. Provato **sabotandolo** con
una scheda finta: diventa rosso e la nomina.
📌 *Una cura si scrive sulla famiglia, non sull'esemplare che si aveva davanti.*
⛔ Le bolle dei messaggi (`.svc-msg`) restano **fuori** di proposito, e c'è un caso di banco apposta:
lì dentro romperebbero *«il messaggio nuovo va in fondo»*, che è una decisione presa.

#### ② La disposizione A

📏 **Misurato prima di disegnare**: nel pannello di destra la scheda è larga **250px** e alta **660**,
e due blocchi da soli ne valgono **210** — Campo **103** (le quattro pastiglie andavano a capo) e
Tipo **107** (griglia 2×2).
⇒ **La data spostata dopo la parola «Campo» non è solo un posto diverso**: stando a destra sulla riga
delle pastiglie se ne prendeva ~55px, ed è *quello* che mandava il campo 4 a capo da solo. La
richiesta sua e il guadagno sono la stessa cosa.
⭐ **I pallini colorati restano**, ed è la ragione per cui la A è stata consigliata contro la B (che
li perdeva in un menu a tendina): il colore del campo è **la stessa lingua del calendario**, dove il
campo si riconosce dal colore prima che dal numero.

| | prima | dopo |
|---|---|---|
| Campo | 103px | **62px** |
| Tipo | 107px | **61px** |
| scheda intera | 660px | **573px** |

🚨⭐ **DUE TRAPPOLE, tutt'e due misurate sulla pagina viva e nessuna delle due deducibile:**
① **un `<button>` dentro una griglia non riceve lo `stretch`**: le colonne erano 51,25px e i bottoni
ne prendevano 43 — la loro larghezza naturale — e **tutte e quattro le parole uscivano troncate coi
puntini** mentre la colonna restava mezza vuota;
② **e nemmeno `flex-grow` lo allarga**: con `flex:1 1 0` il calcolato diceva `flex-grow: 1` e
`flex-basis: 0px`, e i bottoni restavano a 43 su una riga da 220, con **32px di vuoto in coda**.
⇒ Serve `width:100%` + `box-sizing:border-box`, e i campi passano a griglia come i tipi.
📌 *Un bottone non si comporta come un div né in griglia né in flex: la larghezza gliela si dà, non
gliela si chiede.*
🔤 «Manutenzione» si accorcia in **«Manut.»** sotto una certa larghezza, ⛔ ma solo il **testo
mostrato**: `tp.key` resta intero perché è la chiave con cui si cercano le durate
(`SVC_DURATE_PER_TIPO`) e il valore che finisce in `svcFlowState.tipoLabel`, cioè quello che il
riepilogo scrive. È **la stessa distinzione della voce 128**: l'etichetta si traduce, la chiave no.

⏳ **RESTA APERTA**: ✋ un task non è finito finché non lo si è provato fisicamente. Il banco è verde
e le misure sono di TEST 6.291 e PROD 6.278, ma nessuno ha ancora **aperto** una nuova prenotazione
guardandola. ⇒ Si chiude quando lui clicca uno slot libero su PROD e la scheda è come nell'esempio.
...
## 📦 CHIUSE — dal 13/08 al 02/09/2026 — 106 voci
