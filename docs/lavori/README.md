...
### **104** — 🎲 Le opzioni escono sempre in ordine crescente, e quell'ordine È la scala

🆕 **NATA IL 28/08/2026 A MEZZOGIORNO, VOCE SUA**, con lo screenshot della domanda sul vetro davanti:
🗣️ *«Diverse domande sono in ordine crescente di livello: quella più in alto è il livello più basso,
quella in fondo è il più alto. Io questa cosa vorrei cambiarla. Dovrebbe essere un ordine random, se
no dopo un po' la gente capisce questo sistema.»*
🔼 **PROMOSSA IN URGENTI DA LUI** pochi minuti dopo (*«mettila nella lista dei task che stiamo
facendo adesso»*): l'avevo messa in coda io, dichiarando che non bloccava niente, e lui l'ha
spostata di sopra. ⚖️ **Cosa scavalca**: niente — entra accanto alle altre otto.
...
✅ **MISURATO che mescolare è SICURO per il punteggio**: nasce dal **testo**
(`assessmentPublicScoreFromText`), non dall'indice, e `scelte()` tiene il **dato** (`valore`)
attaccato all'etichetta (`testo`) ⇒ spostare un'opzione si porta dietro il suo punteggio. *(Le due
domande sul livello usano `valore = String(f.max)`: anche lì il dato viaggia con l'opzione.)*

🚨⭐⭐ **MA NON SI MESCOLA DENTRO `SCHEDA_DOMANDE`, ed è la trappola.**
`test/motore-a-passi.test.mjs` confronta quelle otto domande con i `<select>` veri di `index.html`
**opzione per opzione e nell'ordine**. Mescolare alla fonte fa diventare rossa quella parità, che
esiste per una ragione buona: finché `index.html` è viva, le domande stanno in **due posti**.
⇒ **Si mescola al momento di presentarle**, in `domandeDelGiro` — dov'è già mescolato l'ordine delle
domande — lasciando `SCHEDA_DOMANDE` come sorgente canonica ordinata.

🔒 **E la mescolata dev'essere RIPETIBILE COL GETTONE**, come quella delle domande: chi lascia un
test a metà deve ritrovare le opzioni dove le aveva lasciate (regola **A3**). Il modo c'è già:
`sorteDa(seme(assessTxt(token), '<sale>'))`.
⚠️ Il **sale dev'essere DIVERSO** da `'ordine-del-giro'` e da quello della pescata — sta scritto nel
commento di `domandeDelGiro`: con lo stesso `rnd` un ordine direbbe qualcosa sull'altro.

⛔ **Le opzioni delle domande di CONOSCENZA non si toccano**: là la risposta giusta è un **indice**
(`correct: N`) e la correzione avviene sul server ripescando col gettone (regola **B7**) ⇒
