...
### **148** — 🪟 SU TELEFONO IL FOGLIO NON SI CHIUDEVA DOPO UN CAMBIO IMPORTO — 🔨 **CURATA, aspetta il suo dito**

🗣️ **Voce SUA**, segnalata dal cellulare il 04/09 subito dopo il **primo cambio importo vero su
PROD 6.330**: *«poi la scheda è rimasta aperta e non si è più chiusa sul cellulare. Quindi ce l'ho
sempre aperta in sovraimpressione.»*

🔎 **DUE IPOTESI CADUTE PRIMA DI TOCCARE NIENTE**, e vale la pena che restino scritte:
· **non era la ✕ coperta dalla fascia della 146.** Misurato sulla pagina viva di PROD a 390 px:
  `elementFromPoint` sul **centro** della ✕ risponde `BUTTON.svc-chat-close-btn`, cioè la ✕ stessa.
  ⭐ La 146 aveva misurato i punti **accanto** alla ✕ (a −4, −8, −14, −20 px); questo è il punto
  **sopra**, che nessuno aveva provato, e regge;
· **non era la 147.** Il suo diff non tocca `svcAutoCloseIfMobile`, `svcMpSyncConfirmed` né la riga
  in questione (**0** occorrenze), e la riga esisteva **identica** su 6.328.

🚨⭐⭐ **NON ERA UN GUASTO: era una riga scritta APPOSTA dalla voce 132** — *«la scheda resta aperta
(i nuovi valori sono già quelli mostrati)»*. Quando il Salva porta **solo** un cambio importo il
ramo torna subito, quindi non passa mai da `svcMpSyncConfirmed` ⇒ nessuno chiama
`svcAutoCloseIfMobile`.
⚖️ **E su DESKTOP quella scelta è giusta**: lì la scheda è un pannello accanto al calendario, e chi
cambia un importo spesso ne cambia un altro. Su **TELEFONO** la stessa scheda è un foglio a **tutto
schermo** che copre il calendario, e «resta aperta» diventa «non se ne va».
📌 *Una scelta giusta su una larghezza non è una scelta: è una scelta più una larghezza sottintesa.
Se la larghezza non è scritta, la si è decisa senza saperlo.*

🔨 **La cura passa da `svcAutoCloseIfMobile`**, che porta dentro le due cose che qui non si possono
perdere: la soglia `< 900px` — il desktop resta **esattamente** com'era e la scelta della 132 non
viene ribaltata, le si **aggiunge la larghezza che le mancava** — e i **700 ms** che lasciano
leggere l'esito.
🚨 **E si chiude SOLO sul verde**, che è la regola di `svcMpSyncConfirmed` applicata qui: se la
scrittura fallisce, l'esito col motivo vive nella riga della scheda, e chiuderla se lo porterebbe
via. *Un foglio che si chiude su un errore è un errore nascosto.*
⛔ **Non si tocca `staffCalPlayersState`**: il gestore della ✕ è `svcCloseChat` e basta, e questa
cura deve fare **quello che farebbe il suo dito** — le differenze fra un gesto e la sua imitazione
si scoprono sempre dopo.

🧪 Banco nuovo `il-foglio-si-chiude-sul-telefono` (5 casi), sabotato **sei** volte — esito buttato ·
chiude anche sul rosso · `svcCloseChat` diretto · soglia 900px tolta · azzera uno stato che la ✕ non
azzera · cura tolta — e cade tutte e sei.
🩹 **E la prima stesura della sonda ritagliava a partire da `await _pmoSetCharges(`**, cioè **dopo**
la riga che il caso ① doveva misurare: dichiarava mancante un `const _esitoCharge =` scritto tre
caratteri prima della fetta. 📌 *Una sonda che ritaglia a partire da ciò che vuole misurare non lo
misura mai.* Ora è ancorata a `const _idResCharge`, con un controllo di distanza.

⏳ **PERCHÉ RESTA APERTA**: la prova è **testuale**. Che sul telefono il foglio si veda chiudere lo
dice il suo dito su un cambio importo **vero**, e quello scrive su Matchpoint ⇒ la console remota,
che entra in sola lettura, non lo può esercitare. È la stessa forma delle voci **65** e **68**.
...
## 📦 CHIUSE — dal 13/08 al 04/09/2026 — 121 voci
