...
🆕📏 **87ª sessione (05/09 notte) — LA FINESTRA SI È APERTA DAVVERO, per 75 minuti, e la cura NON è
stata esercitata lo stesso.** Non un riavvio: l'**avaria della 160** (05/09, 05:57-14:08 locali). Misurato
su tre sonde che non si parlano — i log della piattaforma, la tabella `pmo_eventi_staff`, il registro del
bot:
· fra le **08:53 e le 10:08** locali `consumer-staff-events` ha risposto **57 volte 504 a 150 s** (il
  cancello della piattaforma) e **52 volte 500 a ~74 s** di media (`errore lettura anagrafica: canceling
  statement due to statement timeout`). Il giro del bot parte **ogni 20 s** (`setInterval`, senza guardia
  di sovrapposizione) ⇒ fino a **otto giri in volo insieme**, per un'ora e un quarto;
· in coda c'erano **due fatti** (Fabiola Limuti `tolto` e Lidia Comes `formazione`, 07/09 09:00 C4),
  nati alle **08:24:31** locali — quattro minuti prima dell'avaria — e **letti al passo 1 da ognuno** di
  quei ~109 giri;
· `consegnato_at` di tutt'e due = **14:08:41.849**, cioè **5h44′** dopo, in un giro solo (registro:
  `14:08:42 🔔 circolo: 2 ritirati, 2 detti, 0 scartati`, un solo `detto a` per persona).
⚖️ **Perché lo zero di `gia_presi` qui NON è la firma della cura**: `consegnato_at` dice che **nessuno**
dei 109 giri sovrapposti è arrivato al **passo 5** — i 500 morivano prima, sull'anagrafica, e i 504 li
uccideva il cancello. La corsa che la cura risolve è *due giri che arrivano insieme alla chiusura*;
qui sono arrivati insieme alla **lettura** e nessuno alla chiusura. ⇒ *Fatti letti 109 volte e detti una
volta sola* è vero, ma è merito dell'avaria, non della serratura. La voce resta **aperta e invariata**.
📌 E il modo in cui la finestra si è aperta è nuovo e va saputo: non ai riavvii, dove la scheda la
cercava, ma **ogni volta che il gestionale rallenta** — un giro a passo fisso che non aspetta il
precedente è lo stesso difetto della voce **162** nell'altra stanza (sta fra le «nate misurando»).
...
🆕📏 **87ª sessione (05/09 notte) — LA FINESTRA GUARDATA, ED ERA VUOTA.** Tre sonde:
· **piattaforma** (PROD, 24 ore fino alle 19:42 locali): `consumer-booking-write` **zero chiamate**, quindi
  nessun `KO HTTP` possibile. ⚠️ Ed è un peccato preciso: l'**avaria della 160** (05:57-14:08) era
  l'occasione perfetta per un 504 sul ramo `create`, e in quella finestra **nessun socio ha scritto dal
  bot**;
· **database**: le ricevute dopo la cura del 31/08 sono tutte di scritture **riuscite** — `create` 01/09
  22:39, `entra` 01/09 20:01 e 22:44, `remove` 02/09 09:26 (locali) — e **nessuna** `create-ignoto`,
  `leave-ignoto` o `cancel-ignoto`;
· **registro del bot** (20.000 righe, dal 16/08): l'ultimo `esito IGNOTO` è del **29/08 23:48:58**
  (`[togli] Lidia Comes: rifiutato (esito_ignoto)`, **prima** della cura del 31/08); l'ultimo «Non ci sono
  riuscito» è del **24/08 10:33**. Dopo, niente.
⇒ La voce resta **aperta e invariata**: la cura non ha ancora avuto un 504 da attraversare. Si scrive lo
stesso — *una finestra guardata e trovata vuota è un dato*.
...
precedente, e il passo si allunga quando le risposte rallentano. ⛔ **Non è una voce**: la delega copre
l'ordine, non l'esistenza — e qui si registra la misura, che è ciò da cui una voce può nascere.
...
## 📦 CHIUSE — dal 13/08 al 05/09/2026 — 138 voci
