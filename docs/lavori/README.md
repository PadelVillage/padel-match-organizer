...
| | |
|---|---|
| **163** | 💶🕳️ **L'IMPORTO CHE SI STA DIGITANDO SPARISCE DA SOLO DOPO POCHI SECONDI** — 🆕 *Entrata il 05/09/2026 sera, e non da un'idea: è uscita da una **misura fatta per un'altra voce**.* 📏 Chiudendo la **138** lo screenshot mostrava il campo importo tornato a `0,00` mentre la sonda, un istante prima, ci aveva letto `37,50` — due fatti veri in due momenti diversi. 🔎 Rimisurato apposta su **PROD 6.365**, **senza cliccare niente**: digitato `37,50` nel campo importo della scheda partita, `subito` → **37,50**, `dopo 3 s` → **0,00**, `dopo 10 s` → **0,00**. ⇒ Non lo perde il click sul nome (quello la 138 l'ha provato): lo perde **il ridisegno della scheda** che arriva dopo la rilettura del roster. 🚨 **Perché conta, ed è in cassa che conta**: la segreteria apre la scheda e comincia a scrivere un importo mentre la rilettura è ancora in volo; quando atterra, il numero digitato **sparisce senza dirlo**, e chi stava incassando o riscrive o incassa la cifra sbagliata. ⚖️ È il rovescio del guadagno della **142**: da quando la scheda si apre **subito** coi nomi locali, la finestra in cui si può scrivere **prima** che la rilettura atterri è diventata la normalità invece che l'eccezione. 📌 *Aprire prima una scheda che si ridisegna dopo non è gratis: sposta il ridisegno dentro il tempo in cui qualcuno la sta usando.* ⛔ **COSA QUESTA MISURA NON DICE, e va detto perché decide la cura**: è stata fatta con la console remota, che blocca `/functions/v1/` **di proposito** ⇒ la rilettura dal worker **fallisce subito** e il ridisegno arriva in 3 secondi. Con un worker che risponde davvero il ridisegno arriva **più tardi** — quindi la finestra è più lunga, non più corta — ma **non è misurato** se in quel caso il campo venga svuotato allo stesso modo. Chi prende questa voce lo misuri **prima** di scegliere la cura: con `--allow-writes` no (è la strada verso il Matchpoint vero), ma basta guardarlo dall'app aperta sul Mac. 🔨 **La forma della cura, non ancora decisa**: o il ridisegno **rispetta ciò che l'utente ha in mano** (non tocca i campi toccati, come già fa la nota con `noteChanged`), oppure non ridisegna affatto quelle righe. ⛔ Quello che **non** si fa è togliere la rilettura: è lei a portare i soldi veri. |
...
## 📦 CHIUSE — dal 13/08 al 05/09/2026 — 136 voci
