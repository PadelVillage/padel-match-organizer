...
### **132** — 💶 L'IMPORTO CAMBIATO A MANO DEVE ARRIVARE SU MATCHPOINT **E** SUL GESTIONALE

🗣️ **Voce SUA**, 03/09/2026 mattina: *«quando su una scheda cambio l'importo e poi clicco salva me
lo deve riportare su Matchpoint e sul gestionale»*. 📸 Con schermata di PROD 6.278: la scheda della
partita di Campo 2 · 03/09 · 12:00, con Luca Allera a **10,00 €** e la riga *«Modifica l'importo a
carico se serve, poi incassa»*.

⛔ **NON ANCORA MISURATA** — entra come richiesta sua, e la prima cosa da fare è **guardare cosa
succede oggi**, non supporre. Le domande da chiudere con una misura, prima di scrivere una riga:
① l'importo digitato oggi **dove va a finire** quando si preme Salva — resta nella scheda, va nella
copia locale, o non va da nessuna parte?
② la scrittura dell'importo su Matchpoint **esiste già** nel worker (l'incasso lo imposta prima di
cliccare *Incassare*) oppure va aggiunta?
③ **e soprattutto: «e sul gestionale» in che forma?** Un importo *a carico* non è un pagamento: se
finisse fra i `payment` la sezione Incassi lo conterebbe come **incassato**, che è falso finché
nessuno paga.

🚨⭐⭐ **E LA REGOLA CHE GOVERNA QUESTA VOCE È GIÀ SCRITTA**, in `CLAUDE.md`: *ogni gesto va detto al
socio SOLO DOPO che il circolo l'ha confermato — e nello STESSO ISTANTE dev'essere registrato dal
gestionale.* ⇒ «su Matchpoint **e** sul gestionale» non sono due destinazioni a scelta: sono le due
metà dello stesso passo, e la seconda non può partire prima della conferma della prima.
...
### **133** — 💶 LO STORNO DAGLI INCASSI: una riga per movimento, e si apre la partita

🗣️ **Nasce da una SUA DOMANDA**, 03/09/2026: *«secondo te è possibile negli incassi avere lo storno?
Come lo abbiamo nella tab pagamenti di anagrafica?»* — poi precisata due volte: *«per riga di
movimento»* e *«si clicca sul nome del giocatore e si apre la scheda… così lo storno si fa sempre
nello stesso posto»*.

#### ① Una riga per movimento — e non è estetica

📏 Misurato su PROD, ultimi 7 giorni: **38 righe aggregate contenevano più di un movimento** (fino a
**4**) e **21 ne mescolavano due METODI diversi** — parte Cash, parte Card, sommati in colonne
diverse della **stessa riga**. ⇒ Una riga così **non è un fatto: è due**, e qualunque bottone ci si
metta sopra deve indovinare.
📌 *Una riga su cui si vuole agire deve essere una cosa sola.*
⚖️ Costo: un giorno pieno passa da **35 righe a 49**.

#### ② Si apre la PARTITA, non la scheda socio — e la ragione è una misura

⭐ **Il criterio è suo e resta intero** (*«lo storno si fa sempre nello stesso posto»*); quello che la
misura ha cambiato è **quale** posto:

| | righe (7 giorni) | |
|---|---|---|
| aprono la scheda **socio** | 116 / 250 — **46%** | e tutte quelle collegate la trovano: nessun link rotto |
| sono **«Ospite»** | **120** | una scheda socio non ce l'hanno, e non ce l'avranno mai |
| soci non collegati | 14 | |

⇒ La scheda della **partita** c'è per **tutte**, perché ogni pagamento nasce da una prenotazione.
📌 *Quando una strada copre metà dei casi, la domanda non è come coprire l'altra metà: è se esiste
un'altra strada che le copre tutte.*

#### ③ Il ↩︎ nella scheda della partita è ACCESO — e ribalta una sua decisione, con la sua parola

🚨⭐⭐ **Il buco che la domanda ha fatto emergere**: per un «Ospite» lo storno **non esisteva in
nessun punto del gestionale**. L'unico ↩︎ che li coprirebbe è quello nella scheda della partita, ed
era spento **per sua scelta esplicita del 02/09**. ⇒ Più della metà delle righe di cassa era
scoperta, e sono proprio quelle di chi paga al banco senza essere socio — dove un errore di
battitura è più probabile.
🗣️ Detto **prima** e non dopo, con le tre strade davanti; sua risposta: *«mi hai convinto,
procediamo con la B»*.
⚖️ Ora `_payVoidActive` guarda `PMO_PAYMENTS_VOID_ENABLED`, **lo stesso flag** di `_pmoVoidPayment` e
del ↩︎ della scheda socio: prima ne guardava un altro, e quel bottone poteva restare spento mentre
lo storno era acceso dappertutto. 📌 *Un gesto deve avere UN interruttore: due gesti con un
interruttore solo non si sanno separare, ma un gesto solo con due interruttori non si sa accendere.*

#### 🚨 Quello che NON si fa, ed è deliberato

⛔ La riga **non si porta dietro importo né metodo** del movimento cliccato. 📏 Letto nell'edge
(`marcaStornato`): lo storno filtra per **cliente/nome e SLOT**, mai per metodo o importo ⇒ annulla
**il cobro del giocatore su quella partita**, non il singolo movimento. Precompilare un importo
sarebbe **una promessa che il circolo non mantiene**.
🔪 Il banco lo protegge nei due versi: se un domani l'edge imparasse a stornare per movimento,
diventa rosso — perché quel giorno la riga di Incassi va cambiata **insieme** a lui.

#### 🩹 E la guardia della 128 è diventata rossa, giustamente

Il caso «le chiavi di dato sono intatte» controllava anche `pl.borsellino`, che questa voce ha fatto
sparire **legittimamente** (non si aggrega più per giocatore). ⇒ La cura è stata **guardare se il
fatto reggeva ancora** — i totali vivono in `sel.*` e `d.*` — e riportare la guardia sul fatto, con
un ancoraggio più duraturo: le tre parole che `_incassiMethodBucket` **restituisce**.
📌 *Una guardia che si rompe su un cambio di forma va riportata sul fatto, non allentata: allentarla
è come toglierla, solo più lentamente.*

📏 **Verificato su TEST 6.293** con la console remota: **154 righe di movimento** cliccabili su 5
giorni, l'importo nella **sola** colonna del suo metodo, `onclick` corretto, riepilogo 1.757,00 € su
154 pagamenti, nessun errore di pagina.
⏳ **RESTA APERTA**: manca la prova fisica — un suo click su una riga di cassa, e uno storno vero
portato a termine dalla scheda della partita che si apre.
...
## 📦 CHIUSE — dal 13/08 al 03/09/2026 — 108 voci
