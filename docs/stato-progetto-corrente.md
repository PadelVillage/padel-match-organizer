...
La versione v5.448 e' pubblicata in TEST e PROD al commit app `f7b4814`. La promozione PROD del 2026-05-17 ha portato `main` da app v5.441 a v5.448; rollback tecnico annotato verso `origin/main` precedente `77ba480` / app v5.441 commit `85edd3e`. La modifica v5.448 fa riconoscere `assessment=link-esterno` come accesso pubblico prima della guardia login staff e permette al link pubblico `autovalutazione.html?assessment=link-esterno` e al link TEST `test/autovalutazione.html?env=test&assessment=link-esterno&memberId=PMO-000948` di aprire il flusso pubblico invece della schermata `Padel Match Organizer` con email/password. Non sono stati eseguiti deploy Edge Function, SQL, modifiche scheduler, modifiche segreti, invii email reali, modifiche Matchpoint o dati reali.
...
- Autovalutazione v5.447 TEST/PROD: aggiornati `autovalutazione.html`, `test/autovalutazione.html` e i link copiati dal pannello `Scheda pubblica > Link esterno`. Il branch TEST contiene la preview `Autovalutazione Livello di Gioco`, descrizione `Padel Village` e logo Padel Village; il link pubblico punta alla pagina dedicata `autovalutazione.html?assessment=link-esterno`; il link TEST usa `test/autovalutazione.html?env=test&assessment=link-esterno&memberId=PMO-000948`. La preview reale del link pubblico e' stata portata in PROD con v5.448. Nessuna modifica a SQL, Supabase, Edge Function, scheduler, Matchpoint, Gmail reale, WhatsApp automatico o dati reali.
- Autovalutazione v5.448 TEST/PROD: bugfix routing del link pubblico `Scheda pubblica > Link esterno`; `assessment=link-esterno` viene riconosciuto come accesso pubblico prima della login staff. Il link pubblico PROD e il link TEST aprono il flusso pubblico/registrazione/autovalutazione anche senza sessione staff e non mostrano la schermata login del gestionale. La app normale senza `assessment=link-esterno` mantiene la login staff invariata. Nessuna modifica a SQL, Supabase, Edge Function, scheduler, Matchpoint, Gmail reale, WhatsApp automatico o dati reali.
...
Nota PROD 2026-05-17 14:36: ricevuto comando esplicito `PROMUOVI PROD`, la app v5.448 e' stata promossa in PROD con fast-forward remoto da TEST. La promozione include `index.html`, `autovalutazione.html` e documentazione: link pubblico `Autovalutazione > Scheda pubblica > Link esterno`, preview WhatsApp/Telegram brandizzata e bypass login staff per `assessment=link-esterno`. Non sono stati eseguiti deploy Edge Function, SQL, modifiche scheduler, modifiche Supabase schema, modifiche segreti, invii email reali, modifiche Matchpoint o dati reali. Preflight Supabase PROD: `assessment-email-send` `ACTIVE` v14 `verify_jwt=true`; `cron.job` contiene solo `pmo-data-routines-dispatcher-prod`; nessuno scheduler email Autovalutazione PROD.
