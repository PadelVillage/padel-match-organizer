...
Ultimo aggiornamento: 2026-05-15 16:49
...
| TEST | v5.426 | `test-preview` | `788d1b7` |
| TEST sviluppo | v5.426 | `test/accessi-staff-guidati` | `788d1b7` |

Nota: PROD resta fermo a v5.422. In TEST v5.426 l'invio mattutino Autovalutazione richiede approvazione manuale staff: la routine puo' preparare un lotto fino a 10 email, anche meno se ci sono meno soci pronti, ma non invia senza click `Approva invio`. Il comando `Prepara lotto` / `Approva invio` resta nella testata del `Cruscotto mattutino`; il pulsante `Pulisci` della ricerca del processo ora svuota davvero il campo e mostra di nuovo i filtri, inclusi `Problemi`. Lo scheduler email Autovalutazione PROD resta non attivo.
...
La versione v5.426 e' pubblicata in TEST al commit `788d1b7`; PROD resta a v5.422.
...
- Documentazione aggiornata per v5.426 TEST.
